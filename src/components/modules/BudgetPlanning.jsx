...
const BudgetPlanning = () => {
  ...
  const [budgets, setBudgets] = useState([
    { id:'B-001', deptTr:'Konaklama',           deptEn:'Accommodation',     budget:15000000, actual:12450000 },
    { id:'B-002', deptTr:'Yiyecek & İçecek',    deptEn:'Food & Beverage',   budget:5200000,  actual:4800000  },
    { id:'B-003', deptTr:'SPA & Sağlık',        deptEn:'SPA & Wellness',    budget:1200000,  actual:850000   },
    { id:'B-004', deptTr:'Operasyonel Giderler',deptEn:'Operational Costs', budget:2400000,  actual:2100000  },
    { id:'B-005', deptTr:'Pazarlama',            deptEn:'Marketing',         budget:850000,   actual:920000   },
  ]);

  const [form, setForm] = useState({ deptTr:'', deptEn:'', budget:'', actual:'' });
  ...
  const statusLabel = (s) => {
    if (isEn) return { 'ahead':'Above Target', 'on-track':'On Track', 'behind':'Behind', 'over-budget':'Over Budget' }[s] || s;
    return { 'ahead':'Hedef Üstü', 'on-track':'Planda', 'behind':'Gecikmede', 'over-budget':'Aşım' }[s] || s;
  };
  ...
  const submitBudget = (e) => {
    ...
    addNotification({ type:'info', msg:`${isEn ? 'New budget item added' : 'Yeni bütçe kalemi eklendi'}: ${isEn ? form.deptEn : form.deptTr}` });
    setForm({ deptTr:'', deptEn:'', budget:'', actual:'' });
    setShowForm(false);
  };

  const startEdit = (b) => { setEditing(b.id); setEditBudget(b.budget.toString()); };
  const saveEdit  = (id) => { setBudgets(p => p.map(b => b.id === id ? {...b, budget:Number(editBudget)} : b)); addNotification({ type:'success', msg: isEn ? 'Budget updated' : 'Bütçe güncellendi' }); setEditing(null); };

  const updateActual = (id, amount) => {
    setBudgets(p => p.map(b => b.id === id ? {...b, actual:b.actual+amount} : b));
    addNotification({ type:'info', msg:`${isEn ? 'Actual updated' : 'Gerçekleşen güncellendi'}: +₺${amount.toLocaleString()}` });
  };
  ...
  return (
    <div className="bp-page">
      <div className="bp-head">
        <div>
          <h2><Target size={20}/> {isEn ? 'Budget Planning & Tracking' : 'Bütçe Planlama & Takibi'}</h2>
          <span>{isEn ? 'Annual financial targets and actuals by department' : 'Departman bazlı yıllık finansal hedefler ve gerçekleşme oranları'}</span>
        </div>
        <div className="head-actions">
          <button className="btn-primary" onClick={() => setShowForm(true)}><Plus size={14}/> {isEn ? 'New Budget Item' : 'Yeni Bütçe Kalemi'}</button>
        </div>
      </div>
      ...
      <AnimatePresence>
        {showForm && (
          <motion.form className="budget-form" onSubmit={submitBudget} initial={{opacity:0,y:-10}} animate={{opacity:1,y:0}} exit={{opacity:0}}>
            <div className="bf-head"><h3>{isEn ? 'New Budget Item' : 'Yeni Bütçe Kalemi'}</h3><button type="button" onClick={() => setShowForm(false)}><X size={18}/></button></div>
            <div className="bf-grid">
              <div className="bf"><label>{isEn ? 'Department (TR) *' : 'Departman (TR) *'}</label><input value={form.deptTr} onChange={e => setForm(p => ({...p,deptTr:e.target.value}))} placeholder="Eğlence" required/></div>
              <div className="bf"><label>{isEn ? 'Department (EN) *' : 'Departman (EN) *'}</label><input value={form.deptEn} onChange={e => setForm(p => ({...p,deptEn:e.target.value}))} placeholder="Entertainment" required/></div>
              <div className="bf"><label>{isEn ? 'Annual Budget (₺) *' : 'Yıllık Bütçe (₺) *'}</label><input type="number" value={form.budget} onChange={e => setForm(p => ({...p,budget:e.target.value}))} required/></div>
              <div className="bf"><label>{isEn ? 'Actual (₺)' : 'Gerçekleşen (₺)'}</label><input type="number" value={form.actual} onChange={e => setForm(p => ({...p,actual:e.target.value}))} placeholder="0"/></div>
            </div>
            <div className="bf-foot"><button type="button" className="btn-cancel" onClick={() => setShowForm(false)}>{_c('cancel')}</button><button type="submit" className="btn-primary">{_c('add')}</button></div>
          </motion.form>
        )}
      </AnimatePresence>
      ...
      <div className="bp-global">
        <div className="bg-left">
          <span className="bg-label">{isEn ? 'Total Budget Achievement Rate' : 'Toplam Bütçe Gerçekleşme Oranı'}</span>
        ...
        <div className="bg-right">
          <div className="bg-stat"><span>{isEn ? 'Target' : 'Hedef'}</span><strong>₺{(totalBudget/1000000).toFixed(1)}M</strong></div>
          <div className="bg-stat"><span>{isEn ? 'Actual' : 'Gerçekleşen'}</span><strong>₺{(totalActual/1000000).toFixed(1)}M</strong></div>
        </div>
      </div>
      ...
      <div className="bp-content">
        <div className="dept-grid">
          {budgets.map((b, i) => {
            ...
            return (
              <motion.div key={b.id} className={`depth-card ${status}`} initial={{opacity:0,scale:0.95}} animate={{opacity:1,scale:1}} transition={{delay:i*0.05}}>
                <div className="dc-head">
                  <strong>{isEn ? b.deptEn : b.deptTr}</strong>
                  <div className={`status-pill ${status}`}>{statusLabel(status)}</div>
                </div>
                <div className="dc-metrics">
                  <div className="dcm">
                    <span>{isEn ? 'Budget' : 'Bütçe'}</span>
                    {editing === b.id ? (
                  ...
                  <div className="dcm">
                    <span>{isEn ? 'Actual' : 'Gerçekleşen'}</span>
                    <strong>₺{(b.actual/1000000).toFixed(1)}M</strong>
                  </div>
                </div>
                <div className="dc-progress">
                  <div className="dcp-info"><span>{isEn ? 'Progress' : 'İlerleme'}</span><strong>%{progress}</strong></div>
                ...
                <div className="dc-actions">
                  <button className="dc-btn" onClick={() => startEdit(b)}><Edit3 size={12}/> {isEn ? 'Edit' : 'Düzenle'}</button>
        ...
        <div className="bp-sidebar">
          <div className="comparison-card">
            <h3>{isEn ? 'Annual Comparison' : 'Yıllık Karşılaştırma'}</h3>
            <div className="comp-list">
              {[
                { year: isEn ? '2026 (Forecast)' : '2026 (Tahmin)', val:`₺${(totalBudget/1000000).toFixed(1)}M`, trend:'+12.5%', color:'#3b82f6' },
                { year: isEn ? '2025 (Actual)'  : '2025 (Gerçek)',  val:'₺21.5M',                                 trend:'+8.2%',  color:'#10b981' },
                { year: isEn ? '2024 (Actual)'  : '2024 (Gerçek)',  val:'₺19.8M',                                 trend:'+4.1%',  color:'#94a3b8' },
              ].map(c => (
                <div key={c.year} className="comp-row">
                  <div className="cr-year" style={{borderLeft:`3px solid ${c.color}`}}>
                    <span>{c.year}</span><strong>{c.val}</strong>
                  </div>
          ...
          {overBudgetDepts.length > 0 && (
            <div className="warning-card">
              <div className="wc-head"><AlertCircle size={18} color="#ef4444"/> <strong>{isEn ? 'Budget Alert' : 'Bütçe Alarmı'}</strong></div>
              {overBudgetDepts.map(d => (
                <p key={d.id}>
                  <strong>"{isEn ? d.deptEn : d.deptTr}"</strong>{' '}
                  {isEn
                    ? `department budget exceeded by ${Math.round((d.actual/d.budget-1)*100)}%.`
                    : `${isEn ? 'department budget exceeded by' : 'departmanı bütçesi'} %${Math.round((d.actual/d.budget-1)*100)}${isEn ? '.' : ' oranında aşıldı.'}`}
                </p>
              ))}
            </div>
          )}
          ...
          <div className="dept-summary">
            <h4>{isEn ? 'Department Summary' : 'Departman Sayıları'}</h4>
            <div className="ds-row"><span>{isEn ? 'Total' : 'Toplam'}</span><strong>{budgets.length}</strong></div>
            <div className="ds-row"><span>{isEn ? 'On Track' : 'Planda'}</span><strong style={{color:'#3b82f6'}}>{budgets.filter(b => getStatus(b)==='on-track').length}</strong></div>
            <div className="ds-row"><span>{isEn ? 'Over Budget' : 'Bütçe Aşımı'}</span><strong style={{color:'#ef4444'}}>{overBudgetDepts.length}</strong></div>
          </div>
        </div>
      </div>
      ...
      <style>{`
        ...
        .bf-grid{display:grid;grid-template-columns:1fr 1fr 1fr 1fr;gap:12px;}
