...
const ChannelManager = () => {
  ...
  const [rates, setRates] = useState({ standard: 1850, deluxe: 2400, suite: 4200, family: 3100 });
  const [stopSale, setStopSale] = useState({ Single: false, Double: false, Triple: false });

  const CHANNELS = [
    { id: 'booking', name: 'Booking.com',      statusKey: 'active', sync: isEn ? '5 min ago'   : '5 dk önce',    logo: 'B' },
    { id: 'expedia', name: 'Expedia',           statusKey: 'active', sync: isEn ? '2 min ago'   : '2 dk önce',    logo: 'E' },
    { id: 'hotels',  name: 'Hotels.com',         statusKey: 'active', sync: isEn ? '8 min ago'   : '8 dk önce',    logo: 'H' },
    { id: 'airbnb',  name: 'Airbnb',             statusKey: 'warn',   sync: isEn ? '1 hour ago'  : '1 saat önce',  logo: 'A' },
    { id: 'website', name: isEn ? 'Hotel Website' : 'Otel Web Sitesi', statusKey: 'active', sync: isEn ? 'Live' : 'Anlık', logo: 'W' },
  ];
  ...
  const handleSyncAll = () => {
    ...
    setTimeout(() => {
      setSyncing(false);
      addNotification({ type: 'success', msg: isEn ? 'All channels updated successfully!' : 'Tüm kanallar başarıyla güncellendi!' });
    }, 2000);
  };

  const updateRate = (type, val) => setRates(p => ({ ...p, [type]: parseInt(val) || 0 }));

  return (
    <div className="chan-page">
      <div className="chan-head">
        <div>
          <h2><Layers size={20}/> {isEn ? 'Channel Manager' : 'Kanal Yönetimi (Channel Manager)'}</h2>
          <span>{isEn
            ? 'OTA pricing, allotment and Stop Sale management'
            : 'OTA fiyat, kontenjan ve durdur/satış (Stop Sale) yönetimi'}
          </span>
        </div>
        <button className={`btn-primary ${syncing ? 'syncing' : ''}`} onClick={handleSyncAll} disabled={syncing}>
          <RefreshCw size={15} className={syncing ? 'spin' : ''}/>
          {syncing
            ? (isEn ? 'Synchronizing...' : 'Senkronize Ediliyor...')
            : (isEn ? 'Update All Channels' : 'Tüm Kanalları Güncelle')}
        </button>
      </div>

      <div className="chan-grid">
        <div className="chan-list">
          <h3>{isEn ? 'CONNECTED SALES CHANNELS' : 'BAĞLI SATIŞ KANALLARI'}</h3>
          {channelCounts.map((ch, i) => (
            <motion.div key={ch.id} className={`chan-card ${ch.statusKey}`} initial={{opacity:0,x:-20}} animate={{opacity:1,x:0}} transition={{delay:i*0.1}}>
              <div className="ch-logo">{ch.logo}</div>
              <div className="ch-info">
                <strong>{ch.name}</strong>
                <span>{isEn ? 'Last Sync' : 'Son Senk'}: {ch.sync}</span>
                {ch.resCount > 0 && <span className="ch-res-count">{ch.resCount} {isEn ? 'res.' : 'rez.'}</span>}
              </div>
              <div className={`ch-status ${ch.statusKey}`}>
                {ch.statusKey === 'active' ? <CheckCircle size={14}/> : <AlertCircle size={14}/>}
                {ch.statusKey === 'active'
                  ? (isEn ? 'Connected' : 'Bağlantı Tamam')
                  : (isEn ? 'Sync Error' : 'Senkronizasyon Hatası')}
              </div>
              <button className="ch-btn"><ExternalLink size={14}/></button>
            </motion.div>
          ))}
          <button className="add-chan"><Plus size={14}/> {isEn ? 'Connect New Channel' : 'Yeni Kanal Bağla'}</button>
        </div>

        <div className="chan-main">
          <div className="rate-card">
            <div className="rc-head">
              <h3>{isEn ? 'General Rate Distribution' : 'Genel Fiyat Dağıtımı'}</h3>
              <p>{isEn
                ? 'Changes made here are instantly reflected to all active channels.'
                : 'Burada yapılan değişiklikler tüm aktif kanallara anında yansıtılır.'}
              </p>
            </div>
            ...
            <div className="rate-grid">
              {Object.entries(rates).map(([type, price]) => (
                <div key={type} className="rate-item">
                  <label>{type.toUpperCase()} {isEn ? 'ROOM' : 'ODA'}</label>
                  <div className="price-input">
                    <span>₺</span>
                    <input type="number" value={price} onChange={e => updateRate(type, e.target.value)}/>
                  </div>
                  <div className="rate-meta">
                    <span>{isEn ? 'Previous' : 'Önceki'}: ₺{(price * 0.95).toFixed(0)}</span>
            ...
            <div className="stop-sale-section">
              <h3><Shield size={16}/> {isEn ? 'Stop Sale' : 'Satış Durdurma (Stop Sale)'}</h3>
              <div className="stop-grid">
                {['Single', 'Double', 'Triple'].map(t => (
                  <div key={t} className={`stop-item ${stopSale[t] ? 'stopped' : ''}`}>
                    <span>{t} {isEn ? 'Rooms' : 'Odalar'}</span>
                    <span className="stop-status">{stopSale[t]
                      ? (isEn ? '🛏️ Stopped' : '🛏️ Durduruldu')
                      : (isEn ? '✅ On Sale' : '✅ Satışta')}
                    </span>
                    <div className="toggle">
                      <input type="checkbox" id={`stop-${t}`} checked={stopSale[t]} onChange={() => setStopSale(p => ({...p,[t]:!p[t]}))}/>
                      <label htmlFor={`stop-${t}`}/>
                    </div>
                  </div>
                ))}
              </div>
            ...
            <div className="rc-foot">
              <button className={`btn-save ${saved ? 'saved' : ''}`} onClick={() => {
                setSaved(true);
                addNotification({ type:'success', msg: isEn ? 'Rates and stop-sale sent to all channels!' : 'Fiyatlar ve stop-sale durumu tüm kanallara gönderildi!' });
                setTimeout(() => setSaved(false), 2000);
              }}>
                <Save size={15}/> {saved
                  ? (isEn ? '✓ Sent!' : '✓ Gönderildi!')
                  : (isEn ? 'Send Changes to Channels' : 'Değişiklikleri Kanallara Gönder')}
              </button>
            </div>
          </div>
        </div>
      ...
      <style>{`
        ...
        .chan-list h3{font-size:11px;font-weight:800;color:#64748b;margin-bottom:8px;text-transform:uppercase;letter-spacing:0.5px;}
        ...
        .ch-res-count{display:block;font-size:10px;color:#3b82f6;font-weight:700;}
        .ch-status{font-size:10px;font-weight:800;display:flex;align-items:center;gap:4px;padding:4px 8px;border-radius:20px;}
        .ch-status.active{background:#f0fdf4;color:#10b981;}
        .ch-status.warn{background:#fef2f2;color:#ef4444;}
