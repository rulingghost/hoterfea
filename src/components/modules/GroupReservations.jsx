...
import {
  ...
  Bed, X, ArrowRight, ShieldCheck, Tag, Trash2, Eye, Check
} from 'lucide-react';
...
const GroupReservations = () => {
  ...
  const STATUSES = {
    beklemede:  { label: language === 'tr' ? 'Teklif / Beklemede' : 'Pending / Offer',  color: '#f59e0b', bg: '#fffbeb' },
    konfirm:    { label: language === 'tr' ? 'Onaylandı'         : 'Confirmed',           color: '#3b82f6', bg: '#eff6ff' },
    aktif:      { label: language === 'tr' ? 'Check-in Yapıldı'  : 'Checked In',          color: '#10b981', bg: '#f0fdf4' },
    tamamlandi: { label: language === 'tr' ? 'Tamamlandı'        : 'Completed',           color: '#64748b', bg: '#f1f5f9' },
    iptal:      { label: language === 'tr' ? 'İptal'             : 'Cancelled',           color: '#ef4444', bg: '#fef2f2' },
  };

  const [groups, setGroups] = useState([
    { id:'GRP-001', name:'Germany Tour Group',        leader:'Hans Müller',   pax:45, rooms:22, checkIn:'2026-03-15', checkOut:'2026-03-22', status:'aktif',     total:145000, paid:80000,  board:'AI', notes:'VIP welcome, special menu',       phone:'+49 151 234 5678' },
    { id:'GRP-002', name:'Wedding: Yılmaz & Demir',   leader:'Mehmet Yılmaz', pax:80, rooms:40, checkIn:'2026-03-20', checkOut:'2026-03-22', status:'beklemede', total:320000, paid:50000,  board:'AI', notes:'Hall A — Wedding dinner included', phone:'0532 111 2233' },
    { id:'GRP-003', name:'Ziraat Bank Meeting',        leader:'Deniz Gök',     pax:15, rooms:15, checkIn:'2026-03-25', checkOut:'2026-03-27', status:'konfirm',   total:62500,  paid:62500,  board:'HB', notes:'Meeting room required',            phone:'0212 555 4466' },
    { id:'GRP-004', name:'Japanese Cultural Group',    leader:'Yuki Tanaka',   pax:30, rooms:15, checkIn:'2026-03-28', checkOut:'2026-04-02', status:'beklemede', total:187500, paid:0,      board:'BB', notes:'Japanese-speaking guide requested', phone:'+81 90 1234 5678' },
  ]);
  ...
  const submit = (e) => {
    ...
    addNotification({ type:'info', msg:`${language === 'tr' ? 'Yeni grup rezervasyonu' : 'New group reservation'}: ${form.name} (${form.pax} ${language === 'tr' ? 'kişi' : 'pax'})` });
  ...
  const updateStatus = (id, status) => {
    ...
    const lbl = STATUSES[status]?.label;
    if (status === 'konfirm')    addNotification({ type:'success', msg:`${language === 'tr' ? 'Grup onaylandı' : 'Group confirmed'}: ${grp?.name}` });
    if (status === 'aktif')      addNotification({ type:'success', msg:`${language === 'tr' ? 'Grup check-in yapıldı' : 'Group checked in'}: ${grp?.name}` });
    if (status === 'tamamlandi') addNotification({ type:'info',    msg:`${language === 'tr' ? 'Grup tamamlandı' : 'Group completed'}: ${grp?.name}` });
    if (status === 'iptal')      addNotification({ type:'warn',    msg:`${language === 'tr' ? 'Grup iptal edildi' : 'Group cancelled'}: ${grp?.name}` });
  };

  const deleteGroup = (id) => {
    if (!window.confirm(language === 'tr' ? 'Bu grup rezervasyonunu silmek istediğinize emin misiniz?' : 'Are you sure you want to delete this group reservation?')) return;
    setGroups(p => p.filter(g => g.id !== id));
    addNotification({ type:'info', msg: language === 'tr' ? 'Grup rezervasyonu silindi' : 'Group reservation deleted' });
  };
  ...
  const FILTER_TABS = [
    ...
    { key:'aktif',      label: STATUSES.aktif.label },
    { key:'beklemede',  label: STATUSES.beklemede.label },
    { key:'konfirm',    label: STATUSES.konfirm.label },
    { key:'tamamlandi', label: STATUSES.tamamlandi.label },
  ];
  ...
  return (
    <div className="grp-page">
      <div className="grp-head">
        <div>
          <h2><Users size={20}/> {language === 'tr' ? 'Grup Rezervasyon & Blokaj Yönetimi' : 'Group Reservation & Block Management'}</h2>
          <span>{language === 'tr' ? 'Grup bazlı konaklama, toplu oda atama ve grup foliosu takibi' : 'Group accommodation, bulk room assignment and group folio tracking'}</span>
        </div>
        <button className="btn-primary" onClick={() => setShowForm(!showForm)}>
          <Plus size={15}/> {language === 'tr' ? 'Yeni Grup Rezervasyonu' : 'New Group Reservation'}
        </button>
      </div>

      <div className="grp-stats">
        {[
          { label: language === 'tr' ? 'Aktif Gruplar'   : 'Active Groups',   val: activeGroups,                              icon: <Users size={18}/> },
          { label: language === 'tr' ? 'Bloke Odalar'    : 'Blocked Rooms',   val: totalRooms,                                icon: <Bed size={18}/> },
          { label: language === 'tr' ? 'Toplam Hacim'    : 'Total Volume',    val: `₺${(totalRevenue/1000).toFixed(0)}K`,     icon: <Tag size={18}/> },
        ].map(s => (
          <div key={s.label} className="s-card">
            <div className="sc-icon">{s.icon}</div>
            <div className="sc-info"><span>{s.label}</span><strong>{s.val}</strong></div>
          </div>
        ))}
      </div>
        ...
        {showForm && (
          <motion.form className="grp-form" onSubmit={submit} initial={{opacity:0,y:-10}} animate={{opacity:1,y:0}} exit={{opacity:0}}>
            <div className="gf-head">
              <h3>{language === 'tr' ? 'Yeni Grup Rezervasyonu' : 'New Group Reservation'}</h3>
              <button type="button" onClick={() => setShowForm(false)}><X size={18}/></button>
            </div>
            <div className="gf-grid">
              <div className="gf full"><label>{language === 'tr' ? 'Grup Adı *' : 'Group Name *'}</label><input value={form.name} onChange={e => set('name', e.target.value)} placeholder={language === 'tr' ? 'Ör: Almanya Tur Grubu' : 'e.g. Germany Tour Group'} required/></div>
              <div className="gf"><label>{language === 'tr' ? 'Grup Lideri *' : 'Group Leader *'}</label><input value={form.leader} onChange={e => set('leader', e.target.value)} required/></div>
              <div className="gf"><label>{language === 'tr' ? 'Telefon' : 'Phone'}</label><input value={form.phone} onChange={e => set('phone', e.target.value)} placeholder="+90 5xx..."/></div>
              <div className="gf"><label>{language === 'tr' ? 'Kişi Sayısı *' : 'Pax *'}</label><input type="number" value={form.pax} onChange={e => set('pax', e.target.value)} placeholder="0" required/></div>
              <div className="gf"><label>{language === 'tr' ? 'Oda Sayısı *' : 'Rooms *'}</label><input type="number" value={form.rooms} onChange={e => set('rooms', e.target.value)} placeholder="0" required/></div>
              <div className="gf"><label>{language === 'tr' ? 'Giriş Tarihi *' : 'Check-in *'}</label><input type="date" value={form.checkIn} onChange={e => set('checkIn', e.target.value)} required/></div>
              <div className="gf"><label>{language === 'tr' ? 'Çıkış Tarihi *' : 'Check-out *'}</label><input type="date" value={form.checkOut} onChange={e => set('checkOut', e.target.value)} required/></div>
              <div className="gf"><label>{language === 'tr' ? 'Toplam Tutar (₺) *' : 'Total Amount (₺) *'}</label><input type="number" value={form.total} onChange={e => set('total', e.target.value)} placeholder="0" required/></div>
              <div className="gf"><label>{language === 'tr' ? 'Pansiyon' : 'Board'}</label>
                <select value={form.board} onChange={e => set('board', e.target.value)}>
                  <option value="BB">BB — {language === 'tr' ? 'Oda+Kahvaltı' : 'Bed & Breakfast'}</option>
                  <option value="HB">HB — {language === 'tr' ? 'Yarım Pansiyon' : 'Half Board'}</option>
                  <option value="AI">AI — {language === 'tr' ? 'Her Şey Dahil' : 'All Inclusive'}</option>
                </select>
              </div>
              <div className="gf full"><label>{language === 'tr' ? 'Notlar' : 'Notes'}</label><input value={form.notes} onChange={e => set('notes', e.target.value)} placeholder={language === 'tr' ? 'Özel istekler...' : 'Special requests...'}/></div>
            </div>
            <div className="gf-foot">
              <button type="button" className="btn-cancel" onClick={() => setShowForm(false)}>{_c('cancel')}</button>
              <button type="submit" className="btn-primary">{language === 'tr' ? 'Grup Oluştur' : 'Create Group'}</button>
            </div>
          </motion.form>
        )}
      </AnimatePresence>
      ...
      <div className="grp-main">
        <div className="filters-bar">
          <div className="search-wrap">
            ...
            <input placeholder={language === 'tr' ? 'Grup adı veya lider ara...' : 'Search group or leader...'} value={search} onChange={e => setSearch(e.target.value)}/>
          </div>
          <div className="tabs">
            {FILTER_TABS.map(t => (
              <button key={t.key} className={`t-btn ${filter === t.key ? 'active' : ''}`} onClick={() => setFilter(t.key)}>{t.label}</button>
            ))}
          </div>
        </div>

        <div className="group-list">
          {filtered.length === 0 && <div style={{textAlign:'center',padding:40,color:'#94a3b8',fontSize:14}}>{language === 'tr' ? 'Bu filtrede grup bulunamadı' : 'No groups found for this filter'}</div>}
          {filtered.map((g, i) => {
            ...
            return (
              <motion.div key={g.id} className="group-card" initial={{ opacity:0, x:-10 }} animate={{ opacity:1, x:0 }} transition={{ delay: i * 0.05 }}>
                <div className="gc-main">
                  <div className="gc-info">
                    <div className="gc-id">#{g.id}</div>
                    <strong>{g.name}</strong>
                    <span><Calendar size={11}/> {g.checkIn} → {g.checkOut} ({nights} {language === 'tr' ? 'gece' : 'nights'})</span>
                  </div>
                  <div className="gc-stats-row">
                    <div className="gcs"><span>{language === 'tr' ? 'Oda/Kişi' : 'Room/Pax'}</span><strong>{g.rooms} R / {g.pax} P</strong></div>
                    <div className="gcs"><span>{language === 'tr' ? 'Lider' : 'Leader'}</span><strong>{g.leader}</strong></div>
                  </div>
                </div>
                ...
                <div className="gc-foot">
                  ...
                  <div className="gc-money">
                    ...
                    {g.paid < g.total && <div className="gc-balance">{language === 'tr' ? 'Kalan' : 'Due'}: ₺{(g.total - g.paid).toLocaleString()}</div>}
                  </div>
                  <div className="gc-actions">
                    {g.status === 'beklemede'  && <button className="a-btn confirm"  onClick={() => updateStatus(g.id,'konfirm')}><Check size={14}/> {language === 'tr' ? 'Onayla' : 'Confirm'}</button>}
                    {g.status === 'konfirm'    && <button className="a-btn checkin"  onClick={() => updateStatus(g.id,'aktif')}><ArrowRight size={14}/> Check-in</button>}
                    {g.status === 'aktif'      && <button className="a-btn complete" onClick={() => updateStatus(g.id,'tamamlandi')}><ShieldCheck size={14}/> {language === 'tr' ? 'Tamamla' : 'Complete'}</button>}
        ...
        {detail && (
          <motion.div className="modal-overlay" initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}} onClick={() => setDetail(null)}>
            <motion.div className="modal-box" initial={{scale:0.9}} animate={{scale:1}} onClick={e => e.stopPropagation()}>
              <div className="mb-head"><h3>{language === 'tr' ? 'Grup Detayı' : 'Group Details'} — {detail.name}</h3><button onClick={() => setDetail(null)}><X size={18}/></button></div>
              <div className="mb-body">
                <div className="detail-grid">
                  <div className="dg"><span>Group ID</span><strong>{detail.id}</strong></div>
                  <div className="dg"><span>{language === 'tr' ? 'Lider' : 'Leader'}</span><strong>{detail.leader}</strong></div>
                  <div className="dg"><span>{language === 'tr' ? 'Telefon' : 'Phone'}</span><strong>{detail.phone || '—'}</strong></div>
                  <div className="dg"><span>{language === 'tr' ? 'Pansiyon' : 'Board'}</span><strong>{detail.board}</strong></div>
                  <div className="dg"><span>{language === 'tr' ? 'Kişi Sayısı' : 'Pax'}</span><strong>{detail.pax}</strong></div>
                  <div className="dg"><span>{language === 'tr' ? 'Oda Sayısı' : 'Rooms'}</span><strong>{detail.rooms}</strong></div>
                  <div className="dg"><span>{language === 'tr' ? 'Giriş' : 'Check-in'}</span><strong>{detail.checkIn}</strong></div>
                  <div className="dg"><span>{language === 'tr' ? 'Çıkış' : 'Check-out'}</span><strong>{detail.checkOut}</strong></div>
                  <div className="dg"><span>{language === 'tr' ? 'Toplam Tutar' : 'Total'}</span><strong style={{color:'#10b981'}}>₺{detail.total.toLocaleString()}</strong></div>
                  <div className="dg"><span>{language === 'tr' ? 'Ödenen' : 'Paid'}</span><strong>₺{detail.paid.toLocaleString()}</strong></div>
                  <div className="dg"><span>{language === 'tr' ? 'Kalan Bakiye' : 'Balance'}</span><strong style={{color:detail.total-detail.paid > 0 ? '#ef4444':'#10b981'}}>₺{(detail.total - detail.paid).toLocaleString()}</strong></div>
                  <div className="dg"><span>{language === 'tr' ? 'Durum' : 'Status'}</span><strong>{STATUSES[detail.status]?.label}</strong></div>
                </div>
                {detail.notes && <div className="detail-notes"><strong>{language === 'tr' ? 'Notlar:' : 'Notes:'}</strong> {detail.notes}</div>}
              </div>
      ...
      <style>{`
        ...
        .t-btn{padding:6px 14px;border-radius:8px;border:none;font-size:11px;font-weight:700;color:#64748b;cursor:pointer;white-space:nowrap;background:transparent;}
