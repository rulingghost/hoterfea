...
const ExecutiveDashboard = () => {
  ...
  const d = (key) => mt(`dashboard.${key}`);
  const c = (key) => mt(`common.${key}`);

  const revenueData = [
    { day: d('days').Mon, gelir: 42000, hedef: 38000 },
    { day: d('days').Tue, gelir: 51000, hedef: 38000 },
    { day: d('days').Wed, gelir: 38000, hedef: 38000 },
    { day: d('days').Thu, gelir: 65000, hedef: 38000 },
    { day: d('days').Fri, gelir: 72000, hedef: 38000 },
    { day: d('days').Sat, gelir: 88000, hedef: 38000 },
    { day: d('days').Sun, gelir: 54000, hedef: 38000 },
  ];

  const channelData = [
    { name: 'Booking.com', value: 38, color: '#003580' },
    { name: d('direct'),   value: 28, color: '#3b82f6' },
    { name: 'Expedia',     value: 18, color: '#ef4444' },
    { name: d('other'),    value: 16, color: '#94a3b8' },
  ];

  const statCards = [
    { label: d('occupancy'),    value: `%${stats.occupancyRate}`, sub: d('occupancySub')(stats.occupied, stats.totalRooms), color: '#3b82f6', icon: <Home size={24}/>,        trend: +3.2 },
    { label: d('arrivals'),     value: stats.todayArrivals,       sub: d('arrivalsSub'),    color: '#10b981', icon: <Users size={24}/>,    trend: +5 },
    { label: d('departures'),   value: stats.todayDepartures,     sub: d('departuresSub'), color: '#f59e0b', icon: <Calendar size={24}/>, trend: 0 },
    { label: d('revenue'),      value: `₺${(stats.todayRevenue/1000).toFixed(0)}K`, sub: d('revenueSub'), color: '#8b5cf6', icon: <DollarSign size={24}/>, trend: +12.5 },
    { label: d('inHouse'),      value: stats.inHouse,             sub: d('inHouseSub'),    color: '#ec4899', icon: <Bed size={24}/>,      trend: +8 },
    { label: d('pendingTask'),  value: stats.pendingTasks,        sub: d('pendingTaskSub'), color: '#ef4444', icon: <Clock size={24}/>,   trend: -2 },
  ];

  const roomStats = [
    { label: d('occupied'),    count: stats.occupied,   color: '#3b82f6' },
    { label: d('vacant'),      count: stats.vacant,     color: '#10b981' },
    { label: d('dirty'),       count: stats.dirty,      color: '#f59e0b' },
    { label: d('outOfOrder'),  count: stats.outOfOrder, color: '#ef4444' },
  ];
    ...
    <div className="ed-container">
      ...
      <div className="charts-row">
        <div className="card chart-card wide">
          <div className="card-head">
            <div>
              <h3>{d('weeklyRevenue')}</h3>
              <span>{d('weeklyRevenueSub')}</span>
            </div>
            <div className="total-badge">₺410K {d('weekLabel')}</div>
          </div>
          <div style={{ height: 220 }}>
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={revenueData}>
                ...
                <Area type="monotone" dataKey="hedef" name={d('target')} stroke="#e2e8f0" strokeDasharray="5 5" fill="none" strokeWidth={2}/>
                <Area type="monotone" dataKey="gelir" name={d('actual')} stroke="#3b82f6" fill="url(#gelirGrad)" strokeWidth={3}/>
              </AreaChart>
            </ResponsiveContainer>
          </div>
        </div>
      ...
      <div className="bottom-row">
        ...
        <div className="card table-card">
          <div className="card-head">
            <h3>{d('inHouseGuests')}</h3>
            <span className="badge blue">{stats.inHouse} {c('person')}</span>
          </div>
          <table className="mini-table">
            <thead>
              <tr>
                <th>{c('room')}</th><th>{c('guest')}</th><th>{c('checkIn')}</th>
                <th>{c('checkOut')}</th><th>{c('balance')}</th>
              </tr>
            </thead>
            <tbody>
                ...
                <tr key={r.id}>
                  ...
                  <td>
                    <span className={r.balance > 0 ? 'bal-due' : 'bal-ok'}>
                      {r.balance > 0 ? `₺${r.balance.toLocaleString()} ${c('debt')}` : c('closed')}
                    </span>
                  </td>
                </tr>
              ))}
        ...
        <div className="card room-status-card">
          <div className="card-head"><h3>{d('roomStatus')}</h3></div>
