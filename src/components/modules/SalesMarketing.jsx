...
const SalesMarketing = () => {
  ...
  const STAGES = {
    aday:      { label: isEn ? 'Prospect' : 'Aday',            color: '#94a3b8', bg: '#f1f5f9' },
    görüşme:   { label: isEn ? 'Meeting' : 'Görüşme',          color: '#3b82f6', bg: '#eff6ff' },
    teklif:    { label: isEn ? 'Offer Sent' : 'Teklif Verildi', color: '#f59e0b', bg: '#fffbeb' },
    sözleşme:  { label: isEn ? 'Contract' : 'Sözleşme',        color: '#10b981', bg: '#f0fdf4' },
    kaybedildi:{ label: isEn ? 'Lost' : 'Kaybedildi',          color: '#ef4444', bg: '#fef2f2' },
  };

  const [leads, setLeads] = useState([
    { id:'L-001', name: 'Teknoloji A.Ş.', contact: 'Ali Vural', phone: '+90 532 111 0001', value: 85000, probability: 80, stage: 'teklif', date: '2026-03-14', notes: isEn ? '200-person seminar request' : '200 kişilik seminer talebi' },
    { id:'L-002', name: 'Global Turizm', contact: 'Zeynep Gök', phone: '+90 533 222 0002', value: 142000, probability: 40, stage: 'görüşme', date: '2026-03-12', notes: isEn ? 'Summer season group' : 'Yaz sezonu grubu' },
    { id:'L-003', name: 'Gıda Sanayi Ltd.', contact: 'Mehmet Öz', phone: '+90 534 333 0003', value: 22500, probability: 95, stage: 'sözleşme', date: '2026-03-15', notes: isEn ? 'Employee motivation event' : 'Çalışan motivasyon etkinliği' },
    { id:'L-004', name: 'Eğitim Koçu', contact: 'Merve Hanım', phone: '+90 535 444 0004', value: 12000, probability: 10, stage: 'aday', date: '2026-03-10', notes: isEn ? 'Considering training camp' : 'Eğitim kampı düşünüyor' },
  ]);

  const [campaigns, setCampaigns] = useState([
    { id:'C-001', title: isEn ? 'Holiday Special 20%' : 'Bayram Özel %20',    target: isEn ? 'Individual' : 'Bireysel', color: '#8b5cf6', status: 'aktif', reservations: 124 },
    { id:'C-002', title: isEn ? 'Corporate Discount 2026' : 'Şirket İndirimi 2026', target: isEn ? 'Corporate' : 'Kurumsal', color: '#3b82f6', status: 'aktif', reservations: 87 },
    { id:'C-003', title: isEn ? 'Early Booking' : 'Erken Rezervasyon',        target: isEn ? 'Global' : 'Global',       color: '#10b981', status: 'aktif', reservations: 203 },
  ]);

  const [form, setForm] = useState({ name:'', contact:'', phone:'', value:'', probability:'', stage:'aday', notes:'' });
  const [campForm, setCampForm] = useState({ title:'', target: isEn ? 'Individual' : 'Bireysel', color:'#3b82f6' });
  ...
  const submitLead = (e) => {
    ...
    addNotification({ type:'info', msg: isEn ? `New sales lead: ${form.name} — ₺${Number(form.value).toLocaleString()}` : `Yeni satış fırsatı: ${form.name} — ₺${Number(form.value).toLocaleString()}` });
  ...
  const submitCampaign = (e) => {
    ...
    addNotification({ type:'success', msg: isEn ? `New campaign created: ${campForm.title}` : `Yeni kampanya oluşturuldu: ${campForm.title}` });
    setCampForm({ title:'', target: isEn ? 'Individual' : 'Bireysel', color:'#3b82f6' });
    setShowCampForm(false);
  };

  const updateLeadStage = (id, stage) => {
    setLeads(p => p.map(l => l.id === id ? { ...l, stage } : l));
    addNotification({ type: stage==='sözleşme'?'success':'info', msg: isEn ? `Opportunity status updated → ${STAGES[stage]?.label}` : `Fırsat durumu güncellendi → ${STAGES[stage]?.label}` });
  };
  ...
  return (
    <div className="sm-page">
      <div className="sm-head">
        <div>
          <h2><Megaphone size={20}/> {isEn ? 'Sales & Marketing Management' : 'Satış & Pazarlama Yönetimi'}</h2>
          <span>{isEn ? 'Corporate sales processes, campaign tracking and lead management' : 'Kurumsal satış süreçleri, kampanya takibi ve lead yönetimi'}</span>
        </div>
        <button className="btn-primary" onClick={()=>setShowForm(true)}><Plus size={15}/> {isEn ? 'New Sales Lead' : 'Yeni Satış Fırsatı'}</button>
      </div>

      <AnimatePresence>
        {showForm && (
          <motion.form className="lead-form" onSubmit={submitLead} initial={{opacity:0,y:-10}} animate={{opacity:1,y:0}} exit={{opacity:0}}>
            <div className="lf-head"><h3>{isEn ? 'New Sales Lead' : 'Yeni Satış Fırsatı'}</h3><button type="button" onClick={()=>setShowForm(false)}><X size={18}/></button></div>
            <div className="lf-grid">
              <div className="lf"><label>{isEn ? 'Company Name *' : 'Firma Adı *'}</label><input value={form.name} onChange={e=>setForm(p=>({...p,name:e.target.value}))} placeholder={isEn ? 'Company name' : 'Şirket adı'} required/></div>
              <div className="lf"><label>{isEn ? 'Contact Person *' : 'Yetkili Kişi *'}</label><input value={form.contact} onChange={e=>setForm(p=>({...p,contact:e.target.value}))} placeholder={isEn ? 'Full Name' : 'Ad Soyad'} required/></div>
              <div className="lf"><label>{isEn ? 'Phone' : 'Telefon'}</label><input value={form.phone} onChange={e=>setForm(p=>({...p,phone:e.target.value}))} placeholder="+90 5xx..."/></div>
              <div className="lf"><label>{isEn ? 'Estimated Value (₺) *' : 'Tahmini Değer (₺) *'}</label><input type="number" value={form.value} onChange={e=>setForm(p=>({...p,value:e.target.value}))} required/></div>
              <div className="lf"><label>{isEn ? 'Probability %' : 'Olasılık %'}</label><input type="number" value={form.probability} onChange={e=>setForm(p=>({...p,probability:e.target.value}))} max={100}/></div>
              <div className="lf"><label>{isEn ? 'Stage' : 'Aşama'}</label><select value={form.stage} onChange={e=>setForm(p=>({...p,stage:e.target.value}))}>{Object.entries(STAGES).map(([k,v])=><option key={k} value={k}>{v.label}</option>)}</select></div>
              <div className="lf full"><label>{isEn ? 'Notes' : 'Notlar'}</label><input value={form.notes} onChange={e=>setForm(p=>({...p,notes:e.target.value}))} placeholder={isEn ? 'Details...' : 'Detaylar...'}/></div>
            </div>
            <div className="lf-foot"><button type="button" className="btn-cancel" onClick={()=>setShowForm(false)}>{_c("cancel")}</button><button type="submit" className="btn-primary">{isEn ? 'Save Lead' : 'Fırsat Kaydet'}</button></div>
          </motion.form>
        )}
      </AnimatePresence>
      ...
      <div className="sm-grid">
        <div className="pipeline-section">
          <div className="ps-head">
            <h3>{isEn ? 'Sales Pipeline' : 'Satış Hattı (Pipeline)'}</h3>
            <div className="ps-stats">
              <div className="ps-i"><strong>{leads.length}</strong> {isEn ? 'Leads' : 'Fırsat'}</div>
              <div className="ps-i"><strong>₺{(totalPipeline/1000).toFixed(0)}K</strong> {isEn ? 'Volume' : 'Hacim'}</div>
            </div>
          </div>
          <div className="lead-list">
            {leads.map((l, i) => {
              ...
              return (
                <motion.div key={l.id} className="lead-card" initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.05 }}>
                  ...
                  <div className="lc-mid">
                    ...
                    <div className="lc-prob">
                      <span>{isEn ? 'Probability' : 'Olasılık'}: %{l.probability}</span>
                  ...
                  <div className="lc-foot">
                    ...
                    <div className="lc-actions">
                      {l.stage === 'aday' && <button className="la blue" onClick={()=>updateLeadStage(l.id,'görüşme')}>{isEn ? 'Meeting' : 'Görüşme'}</button>}
                      {l.stage === 'görüşme' && <button className="la amber" onClick={()=>updateLeadStage(l.id,'teklif')}>{isEn ? 'Send Offer' : 'Teklif Ver'}</button>}
                      {l.stage === 'teklif' && <button className="la green" onClick={()=>updateLeadStage(l.id,'sözleşme')}>{isEn ? 'Contract' : 'Sözleşme'}</button>}
                      {l.stage !== 'sözleşme' && l.stage !== 'kaybedildi' && <button className="la red" onClick={()=>updateLeadStage(l.id,'kaybedildi')}>{isEn ? 'Lost' : 'Kaybet'}</button>}
        ...
        <div className="sm-sidebar">
          <div className="camp-card-main">
            <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:20}}>
              <h3 style={{marginBottom:0}}>{isEn ? 'Active Campaigns' : 'Aktif Kampanyalar'}</h3>
            ...
            <AnimatePresence>
              {showCampForm && (
                <motion.form className="camp-form" onSubmit={submitCampaign} initial={{opacity:0,height:0}} animate={{opacity:1,height:'auto'}} exit={{opacity:0,height:0}}>
                  <input value={campForm.title} onChange={e=>setCampForm(p=>({...p,title:e.target.value}))} placeholder={isEn ? 'Campaign name *' : 'Kampanya adı *'} required/>
                  <select value={campForm.target} onChange={e=>setCampForm(p=>({...p,target:e.target.value}))}>
                    <option>{isEn ? 'Individual' : 'Bireysel'}</option>
                    <option>{isEn ? 'Corporate' : 'Kurumsal'}</option>
              ...
              {campaigns.map(c => (
                <div key={c.id} className="ci">
                  ...
                  <div className="ci-info">
                    ...
                    <span>{isEn ? 'Target' : 'Hedef'}: {c.target}</span>
                  </div>
                  <div className="ci-stat">{c.reservations} {isEn ? 'Res.' : 'Rez.'}</div>
          ...
          <div className="sales-stats-c">
            <div className="ssc-head">{isEn ? 'Sales Performance' : 'Satış Performansı'}</div>
            <div className="ssc-metrics">
              <div className="sscm">
                <span className="sscm-l">{isEn ? 'Conversion Rate' : 'Dönüşüm Oranı'}</span>
              ...
              <div className="sscm">
                <span className="sscm-l">{isEn ? 'Pipeline Volume' : 'Pipeline Hacmi'}</span>
                <div className="sscm-v">₺{(totalPipeline/1000).toFixed(0)}K</div>
              </div>
              <div className="sscm">
                <span className="sscm-l">{isEn ? 'Active Leads' : 'Aktif Fırsatlar'}</span>
          ...
          <motion.div className="modal-overlay" initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}} onClick={()=>setDetail(null)}>
            <motion.div className="modal-box" initial={{scale:0.9}} animate={{scale:1}} onClick={e=>e.stopPropagation()}>
              ...
              <div className="detail-grid">
                <div className="dg"><span>{isEn ? 'Contact' : 'Yetkili'}</span><strong>{detail.contact}</strong></div>
                <div className="dg"><span>{isEn ? 'Phone' : 'Telefon'}</span><strong>{detail.phone || '—'}</strong></div>
                <div className="dg"><span>{isEn ? 'Value' : 'Değer'}</span><strong style={{color:'#10b981'}}>₺{detail.value.toLocaleString()}</strong></div>
                <div className="dg"><span>{isEn ? 'Probability' : 'Olasılık'}</span><strong>%{detail.probability}</strong></div>
                <div className="dg"><span>{isEn ? 'Stage' : 'Aşama'}</span><strong style={{color:STAGES[detail.stage]?.color}}>{STAGES[detail.stage]?.label}</strong></div>
                <div className="dg"><span>{isEn ? 'Date' : 'Tarih'}</span><strong>{detail.date}</strong></div>
              </div>
