...
const NewReservationWizard = () => {
  ...
  const STEPS = isEn
    ? ['Guest Info', 'Room & Dates', 'Pricing', 'Confirm']
    : ['Misafir Bilgileri', 'Oda & Tarih', 'Fiyatlandırma', 'Onay'];

  const BOARDS = isEn ? [
    { code:'BB', label:'Bed & Breakfast', desc:'Breakfast only' },
    { code:'HB', label:'Half Board',      desc:'Breakfast + dinner' },
    { code:'FB', label:'Full Board',      desc:'3 meals included' },
    { code:'AI', label:'All Inclusive',   desc:'Unlimited F&B' },
  ] : [
  ...
  const CHANNELS = ['Direct','Booking.com','Expedia','HotelRunner','TUI','Agency'];

  const PAY_METHODS = isEn
    ? ['Cash','Credit Card','Bank Transfer','Open Account']
    : ['Nakit','Kredi Kartı','EFT/Havale','Açık Hesap'];

  const [form, setForm] = useState({
    guest:'', phone:'', email:'', nationality:'TR', pax:2,
    room:'', checkIn:'2026-03-14', checkOut:'2026-03-17', board:'HB',
    channel:'Direct', paid:0, payMethod:PAY_METHODS[1], notes:''
  });
  ...
  const reset = () => {
    ...
    setForm({ guest:'', phone:'', email:'', nationality:'TR', pax:2, room:'', checkIn:'2026-03-14', checkOut:'2026-03-17', board:'HB', channel:'Direct', paid:0, payMethod:PAY_METHODS[1], notes:'' });
  };
  ...
  if (done) return (
    <div className="wizard-done">
      ...
      <h2>{isEn ? 'Reservation Created!' : 'Rezervasyon Oluşturuldu!'}</h2>
      <p>{form.guest} — {nights} {isEn ? 'nights' : 'gece'} · {isEn ? 'Room' : 'Oda'} {form.room} · ₺{total.toLocaleString()}</p>
      <div className="done-actions">
        <button className="btn-outline" onClick={reset}><BookOpen size={16}/> {isEn ? 'New Reservation' : 'Yeni Rezervasyon'}</button>
      </div>
    </div>
  );
  ...
  return (
    <div className="wizard-page">
      <div className="wizard-header">
        <h2>{isEn ? 'New Reservation Wizard' : 'Yeni Rezervasyon Sihirbazı'}</h2>
        <p>{isEn ? 'Step' : 'Adım'} {step+1}/{STEPS.length} — {STEPS[step]}</p>
      </div>
      ...
      <div className="wizard-body">
        <AnimatePresence mode="wait">
          <motion.div key={step} initial={{opacity:0,x:30}} animate={{opacity:1,x:0}} exit={{opacity:0,x:-30}} className="wizard-panel">
            ...
            {step===0 && (
              <div className="wstep">
                <h3><User size={18}/> {isEn ? 'Guest Information' : 'Misafir Bilgileri'}</h3>
                <div className="form-grid">
                  <div className="fg full"><label>{isEn ? 'Full Name *' : 'Ad Soyad *'}</label>
                    <input value={form.guest} onChange={e => sel('guest', e.target.value)} placeholder={isEn ? 'Guest full name' : 'Misafir adı soyadı'}/>
                      ...
                      <div className="guest-suggest">
                        {guests.filter(g => g.name.toLowerCase().includes(form.guest.toLowerCase())).slice(0,4).map(g => (
                          <button key={g.id} className="sug-item" onClick={() => sel('guest', g.name)}>
                            <strong>{g.name}</strong><span>{g.loyalty} · {g.visits} {isEn ? 'stays' : 'konaklama'}</span>
                          </button>
                        ))}
                      </div>
                    )}
                  </div>
                  <div className="fg"><label>{isEn ? 'Phone' : 'Telefon'}</label><input value={form.phone} onChange={e => sel('phone', e.target.value)} placeholder="+90 5xx xxx xx xx"/></div>
                  <div className="fg"><label>{isEn ? 'Email' : 'E-posta'}</label><input value={form.email} onChange={e => sel('email', e.target.value)} placeholder="email@example.com"/></div>
                  <div className="fg"><label>{isEn ? 'Nationality' : 'Uyruk'}</label>
                  ...
                  <div className="fg"><label>{isEn ? 'Pax' : 'Kişi Sayısı'}</label><input type="number" value={form.pax} onChange={e => sel('pax', e.target.value)} min={1} max={8}/></div>
                  <div className="fg full"><label>{isEn ? 'Notes' : 'Notlar'}</label><textarea value={form.notes} onChange={e => sel('notes', e.target.value)} placeholder={isEn ? 'VIP, special requests, etc.' : 'VIP, özel istek, vb.'} rows={2}/></div>
                </div>
              </div>
            )}
            ...
            {step===1 && (
              <div className="wstep">
                <h3><Bed size={18}/> {isEn ? 'Room & Date Selection' : 'Oda & Tarih Seçimi'}</h3>
                <div className="date-grid">
                  <div className="fg"><label>{isEn ? 'Check-in *' : 'Giriş Tarihi *'}</label><input type="date" value={form.checkIn} onChange={e => sel('checkIn', e.target.value)}/></div>
                  <div className="fg"><label>{isEn ? 'Check-out *' : 'Çıkış Tarihi *'}</label><input type="date" value={form.checkOut} onChange={e => sel('checkOut', e.target.value)}/></div>
                  {nights > 0 && <div className="nights-badge">{nights} {isEn ? 'Nights' : 'Gece'}</div>}
                </div>
                <label className="sec-label">{isEn ? 'Available Rooms' : 'Müsait Odalar'} ({freeRooms.length} {isEn ? 'rooms' : 'oda'})</label>
                <div className="room-grid">
                  {freeRooms.map(r => (
                    <button key={r.id} className={`room-card ${form.room === r.id ? 'active' : ''}`} onClick={() => sel('room', r.id)}>
                      ...
                      <div className="rc-rate">₺{r.rate}<small>/{isEn ? 'night' : 'gece'}</small></div>
                      {nights > 0 && <div className="rc-total">₺{(r.rate*nights).toLocaleString()} {isEn ? 'total' : 'toplam'}</div>}
            ...
            {step===2 && (
              <div className="wstep">
                <h3><CreditCard size={18}/> {isEn ? 'Pricing & Channel' : 'Fiyatlandırma & Kanal'}</h3>
                <div className="form-grid">
                  <div className="fg full">
                    <label>{isEn ? 'Board Type' : 'Pansiyon Tipi'}</label>
                  ...
                  <div className="fg full">
                    <label>{isEn ? 'Booking Channel' : 'Rezervasyon Kanalı'}</label>
                  ...
                  <div className="fg"><label>{isEn ? 'Deposit (₺)' : 'Ön Ödeme (₺)'}</label>
                    <input type="number" value={form.paid} onChange={e => sel('paid', e.target.value)} min={0} max={total}/>
                  </div>
                  <div className="fg"><label>{isEn ? 'Payment Method' : 'Ödeme Yöntemi'}</label>
            ...
            {step===3 && (
              <div className="wstep">
                <h3><CheckCircle size={18}/> {isEn ? 'Reservation Summary' : 'Rezervasyon Özeti'}</h3>
                <div className="confirm-grid">
                  <div className="cg-row"><span>{isEn ? 'Guest' : 'Misafir'}</span><strong>{form.guest}</strong></div>
                  <div className="cg-row"><span>{isEn ? 'Room' : 'Oda'}</span><strong>{form.room} — {selRoom?.type}</strong></div>
                  <div className="cg-row"><span>{isEn ? 'Check-in / Check-out' : 'Giriş / Çıkış'}</span><strong>{form.checkIn} → {form.checkOut} ({nights} {isEn ? 'nights' : 'gece'})</strong></div>
                  <div className="cg-row"><span>{isEn ? 'Pax' : 'Kişi Sayısı'}</span><strong>{form.pax}</strong></div>
                  <div className="cg-row"><span>{isEn ? 'Board' : 'Pansiyon'}</span><strong>{BOARDS.find(b => b.code === form.board)?.label}</strong></div>
                  <div className="cg-row"><span>{isEn ? 'Channel' : 'Kanal'}</span><strong>{form.channel}</strong></div>
                  <div className="cg-row price"><span>{isEn ? 'Total Amount' : 'Toplam Tutar'}</span><strong>₺{total.toLocaleString()}</strong></div>
                  <div className="cg-row"><span>{isEn ? 'Deposit' : 'Ön Ödeme'}</span><strong className="green">₺{Number(form.paid).toLocaleString()}</strong></div>
                  <div className="cg-row"><span>{isEn ? 'Outstanding' : 'Kalan Bakiye'}</span><strong className="red">₺{Math.max(0, total - Number(form.paid)).toLocaleString()}</strong></div>
                  {form.notes && <div className="cg-row"><span>{isEn ? 'Notes' : 'Notlar'}</span><strong>{form.notes}</strong></div>}
                </div>
              </div>
            )}
          </motion.div>
      ...
      <div className="wizard-nav">
        <button className="btn-back" onClick={() => setStep(s => s-1)} disabled={step===0}>← {isEn ? 'Back' : 'Geri'}</button>
        <div className="dots">{STEPS.map((_, i) => <div key={i} className={`dot ${i===step?'active':''}`}/>)}</div>
        {step < STEPS.length-1
          ? <button className="btn-next-w" onClick={() => setStep(s => s+1)} disabled={!canNext}>{isEn ? 'Next' : 'İleri'} <ArrowRight size={15}/></button>
          : <button className="btn-finish" onClick={handleSubmit}>✓ {isEn ? 'Create Reservation' : 'Rezervasyonu Oluştur'}</button>
        }
      </div>
      ...
      <style>{`
        ...
        .room-card:hover,.room-card.active{border-color:#3b82f6;background:#eff6ff;}
        ...
        .cg-row.price{background:#eff6ff;margin:-1px 0;padding:14px 0;}
