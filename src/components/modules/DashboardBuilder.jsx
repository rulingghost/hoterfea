...
const DashboardBuilder = () => {
  ...
  const dataSources = isEn ? [
    { name: 'Occupancy & Revenue',  icon: <BarChart3 size={16}/> },
    { name: 'Survey Results',       icon: <LineIcon size={16}/> },
    { name: 'POS & Sales',          icon: <PieIcon size={16}/> },
    { name: 'Reviews & NPS',        icon: <Share size={16}/> },
    { name: 'Channel Manager',      icon: <Zap size={16}/> },
    { name: 'Procurement',          icon: <Layout size={16}/>, isNew: true },
    ...
    { name: 'Satın Alma',       icon: <Layout size={16}/>, isNew: true },
    { name: 'Spa & Wellness',   icon: <Palette size={16}/> },
  ];
    ...
    <div className="builder-container">
      <header className="header">
        <div className="title-section">
          ...
          <div>
            <h2>{isEn ? 'Custom Dashboard Builder' : 'Özel Dashboard Oluşturucu'}</h2>
            <span>{isEn
              ? 'Drag-and-drop widget interface, dynamic data sources and report generation'
              : 'Sürükle-bırak widget arayüzü, dinamik veri kaynakları ve rapor oluşturma'}
            </span>
          </div>
        </div>
        <div className="actions">
          <button className="btn outline"><Save size={18}/> {isEn ? 'SAVE & PUBLISH' : 'KAYDET VE YAYINLA'}</button>
          <button className="btn outline"><FileJson size={18}/> {isEn ? 'EXPORT JSON' : 'JSON DIŞA AKTAR'}</button>
          <button className="btn primary red">{isEn ? 'SWITCH TO LIVE' : 'CANLI MODA GEÇ'}</button>
        </div>
      </header>
      ...
      <div className="builder-grid">
        <aside className="left-panel">
          <section className="card sources-card">
            <h3>{isEn ? 'DATA SOURCES' : 'VERİ KAYNAKLARI'}</h3>
            <div className="source-list">
              {dataSources.map((s, i) => (
                <div key={i} className="source-item">
                  {s.icon}
                  <span>{s.name}</span>
                  {s.isNew && <div className="new-tag">{isEn ? 'NEW' : 'YENİ'}</div>}
                </div>
              ))}
            </div>
          </section>
        ...
        <section className="canvas-area">
          <div className="canvas-header">
            <div className="search-box">
              ...
              <input type="text" placeholder={isEn ? 'Search widget...' : 'Widget Ara...'} />
            </div>
          </div>
          ...
          <div className="canvas-grid">
            <div className="widget preview-widget wider">
              <div className="w-head">
                <span>{isEn ? 'Monthly Revenue Statistics' : 'Aylık Ciro İstatistikleri'}</span>
                <strong>$920, 9%</strong>
              </div>
            ...
            <div className="widget preview-widget">
              <div className="w-head">
                <span>{isEn ? 'Occupancy Gauge' : 'Doluluk Göstergesi'}</span>
              </div>
              <div className="gauge-placeholder">
                <div className="gauge-inner">
                  <strong>92%</strong>
                  <span>{isEn ? 'Today' : 'Bugün'}</span>
                </div>
              </div>
          ...
          <div className="canvas-footer">
            <span>{isEn ? 'Workspace' : 'Çalışma Alanı'}: <strong>{isEn ? 'General Manager Panel' : 'Genel Müdür Paneli'}</strong></span>
            <span>{isEn ? 'Last saved: 1 min ago' : 'Son Kayıt: 1 dak. Önce'}</span>
          </div>
        </section>
        ...
        <aside className="right-panel">
          <section className="card props-card">
            ...
            <h3>{isEn ? 'WIDGET PROPERTIES' : 'WİDGET ÖZELLİKLERİ'}</h3>
            <div className="prop-section">
              <span className="p-label">{isEn ? 'Colors' : 'Renkler'}</span>
              <div className="color-dots">
                {['#3b82f6','#84cc16','#f59e0b','#ef4444','#d946ef','#8b5cf6'].map(c => (
                  <div key={c} className="c-dot" style={{ background: c }}/>
                ))}
              </div>
            </div>

            <div className="prop-section mt-20">
              <span className="p-label">{isEn ? 'Data Time Range' : 'Veri Zaman Aralığı'}</span>
              <select className="p-select">
                <option>{isEn ? 'Real-time' : 'Gerçek Zamanlı'}</option>
                <option>{isEn ? 'Today' : 'Bugün'}</option>
                <option>{isEn ? 'Last 7 Days' : 'Son 7 Gün'}</option>
                <option>{isEn ? 'This Month' : 'Bu Ay'}</option>
                <option>Custom</option>
              </select>
            </div>

            <div className="prop-section mt-20">
              <span className="p-label">{isEn ? 'Refresh Frequency' : 'Yenileme Sıklığı'}</span>
              <select className="p-select">
                <option>{isEn ? 'Every 1 Minute' : 'Her 1 Dakika'}</option>
                <option>{isEn ? '2 Minutes' : '2 Dakika'}</option>
                <option>{isEn ? '5 Minutes' : '5 Dakika'}</option>
              </select>
            </div>

            <div className="auto-save mt-20">
              <div className="s-head">
                <span>{isEn ? 'Auto-save' : 'Otomatik'}</span>
      ...
      <style>{`
        .builder-container { padding: 30px; background: #f1f5f9; height: calc(100vh - 70px); overflow-y: auto; display: flex; flex-direction: column; gap: 30px; }
