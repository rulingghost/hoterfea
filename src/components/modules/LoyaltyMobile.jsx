...
import {
  ...
  UserCheck, Trophy, Bell, ExternalLink, ChevronRight, Plus
} from 'lucide-react';
  ...
  const levels = [
    {
      ...
      points: '0 - 1000',
      perks: isEn ? ['5% Discount', 'Late Check-out'] : ['%5 İndirim', 'Geç Check-out'],
      count: 142
    },
    {
      name: 'Gold', color: '#f59e0b',
      points: '1001 - 5000',
      perks: isEn ? ['10% Discount', 'Room Upgrade', 'Free Breakfast'] : ['%10 İndirim', 'Oda Upgrade', 'Ücretsiz Kahvaltı'],
      count: 45
    },
    {
      name: 'Platinum', color: '#3b82f6',
      points: '5001+',
      perks: isEn ? ['20% Discount', 'VIP Transfer', 'SPA Access'] : ['%20 İndirim', 'VIP Transfer', 'SPA Kullanımı'],
      count: 12
    },
  ];

  const mobileRequests = [
    { id:'REQ-1', guest:'Ahmet Yılmaz', type:'Room Service',  titleTr:'Ekstra Yastık',   titleEn:'Extra Pillow',   status:'yeni',        time:'14:20' },
    { id:'REQ-2', guest:'Sarah Johnson', type:'Housekeeping', titleTr:'Oda Temizliği',   titleEn:'Room Cleaning',  status:'hazırlanıyor', time:'14:15' },
    { id:'REQ-3', guest:'Klaus Weber',   type:'Concierge',    titleTr:'Taksi Çağır',     titleEn:'Call a Taxi',    status:'tamamlandı',   time:'13:50' },
  ];

  const STATUS_LABEL = {
    yeni:          isEn ? 'New'         : 'Yeni',
    hazırlanıyor:  isEn ? 'In Progress' : 'Hazırlanıyor',
    tamamlandı:    isEn ? 'Done'        : 'Tamamlandı',
  };
  ...
  return (
    <div className="loy-page">
      <div className="loy-head">
        <div>
          <h2><Heart size={20}/> {isEn ? 'Loyalty & Mobile App' : 'Sadakat & Mobil Uygulama'}</h2>
          <span>{isEn ? 'Guest loyalty, points system and mobile app requests' : 'Misafir bağlılığı, puan sistemi ve mobil uygulama talepleri'}</span>
        </div>
        <div className="tab-switcher">
          <button className={activeTab === 'program' ? 'active' : ''} onClick={() => setActiveTab('program')}>
            {isEn ? 'Loyalty Program' : 'Sadakat Programı'}
          </button>
          <button className={activeTab === 'mobile'  ? 'active' : ''} onClick={() => setActiveTab('mobile')}>
            {isEn ? 'Mobile Requests' : 'Mobil Talepler'}
          </button>
        </div>
      </div>
          ...
          <motion.div key="program" className="loy-content" initial={{opacity:0,y:10}} animate={{opacity:1,y:0}} exit={{opacity:0,y:-10}}>
              ...
              {levels.map(lvl => (
                <div key={lvl.name} className="level-card">
                  <div className="lc-head">
                    ...
                    <div>
                      <h3>{lvl.name} {isEn ? 'Members' : 'Üyeler'}</h3>
                      <span>{lvl.points} {isEn ? 'Points' : 'Puan'}</span>
                    </div>
                    <div className="count-pill">{lvl.count} {isEn ? 'Members' : 'Üye'}</div>
                  </div>
                  <ul className="perk-list">
                    {lvl.perks.map(p => <li key={p}><Star size={10}/> {p}</li>)}
                  </ul>
                  <button className="lc-btn">{isEn ? 'List Members' : 'Üyeleri Listele'}</button>
                </div>
              ))}
            </div>
            ...
            <div className="loy-stats-row">
              <div className="stat-box">
                <div className="sb-label">{isEn ? 'Total Loyalty Points' : 'Toplam Sadakat Puanı'}</div>
                <div className="sb-value">1,420,500</div>
                <div className="sb-trend up">+12.4%</div>
              </div>
              <div className="stat-box">
                <div className="sb-label">{isEn ? 'Points Redeemed' : 'Harcanan Puanlar'}</div>
                <div className="sb-value">₺125,000</div>
                <div className="sb-trend down">-4.2%</div>
              </div>
              <div className="stat-box">
                <div className="sb-label">{isEn ? 'App Downloads' : 'Mobil Uygulama İndirme'}</div>
            ...
            <div className="campaign-section">
              <h3><Gift size={16}/> {isEn ? 'Active Campaigns' : 'Aktif Kampanyalar'}</h3>
              <div className="camp-grid">
                {[
                  { titleTr:'Yaz Tatili %15 Puan', titleEn:'Summer +15% Points',      descTr:'SPA harcamalarında 2 kat puan kazanma fırsatı.',              descEn:'Double points on all SPA spending.',          type:'SPA' },
                  { titleTr:'Hafta sonu Kaçamağı', titleEn:'Weekend Getaway Offer',   descTr:'3 gece kal 2 gece öde — mobil üyelerinize özel.',              descEn:'Stay 3 nights pay 2 — exclusive for app users.', type:isEn?'Accommodation':'Konaklama' },
                ].map(c => (
                  <div key={c.titleEn} className="camp-card">
                    <div className="camp-icon"><Smartphone size={18}/></div>
                    <div className="camp-info">
                      <strong>{isEn ? c.titleEn : c.titleTr}</strong>
                      <p>{isEn ? c.descEn : c.descTr}</p>
                    </div>
                    <button className="edit-btn">{isEn ? 'Edit' : 'Düzenle'}</button>
                  </div>
                ))}
                <button className="add-camp"><Plus size={14}/> {isEn ? 'New Campaign' : 'Yeni Kampanya Oluştur'}</button>
              </div>
            </div>
          </motion.div>
        ) : (
            ...
            <div className="mobile-main">
              <div className="mobile-requests">
                <div className="mr-head">
                  <h3>{isEn ? 'Active Mobile Requests' : 'Aktif Mobil Talepler'}</h3>
                  <div className="badge blue">{mobileRequests.filter(r => r.status !== 'tamamlandı').length} {isEn ? 'Pending' : 'Bekleyen'}</div>
                </div>
                  ...
                  {mobileRequests.map(req => (
                    <div key={req.id} className={`req-item ${req.status}`}>
                      ...
                      <div className="req-info">
                        ...
                        <span>{req.type} — {isEn ? req.titleEn : req.titleTr}</span>
                      </div>
                      <div className="req-meta">
                        <div className="req-time">{req.time}</div>
                        <div className={`status-tag ${req.status}`}>{STATUS_LABEL[req.status]}</div>
                      </div>
              ...
              <div className="mobile-preview">
                <div className="phone-mockup">
                  <div className="mock-screen">
                    <div className="app-header">{isEn ? 'Hotel App' : 'Otel Uygulaması'}</div>
                    <div className="app-body">
                      <div className="app-card guest">{isEn ? 'Welcome, ' : 'Hoş Geldiniz, '}<strong>{isEn ? 'Mr. Yılmaz' : 'Sayın Yılmaz'}</strong></div>
                      <div className="app-menu">
                        <div className="menu-i">📞 {isEn ? 'Reception' : 'Resepsiyon'}</div>
                        <div className="menu-i">🧹 {isEn ? 'Housekeeping' : 'Oda Temizliği'}</div>
                        <div className="menu-i">🍽️ {isEn ? 'Room Service' : 'Oda Servisi'}</div>
                        <div className="menu-i">💆 SPA</div>
                      </div>
                    </div>
                  </div>
                </div>
                <div className="mock-actions">
                  <h4>{isEn ? 'App Settings' : 'Uygulama Ayarları'}</h4>
                  <button className="mbtn"><MessageSquare size={14}/> {isEn ? 'Send Push Notification' : 'Push Bildirimi Gönder'}</button>
                  <button className="mbtn"><ExternalLink size={14}/> {isEn ? 'Send App Link' : 'Uygulama Linkini Gönder'}</button>
                </div>
              </div>
            </div>
          </motion.div>
      ...
      <style>{`
        ...
        .sb-trend.up{background:#f0fdf4;color:#10b981;}.sb-trend.down{background:#fef2f2;color:#ef4444;}
