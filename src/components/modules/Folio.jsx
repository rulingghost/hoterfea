...
import {
  FileText, Plus, Trash2, CreditCard,
  Printer, CheckCircle, AlertCircle, X, Search
} from 'lucide-react';

// Sub-components defined outside to avoid hook-in-render issues
const PaymentModal = ({ res, onClose, onPay, language }) => {
  const [amount, setAmount] = useState(res.balance);
  const [method, setMethod] = useState(language === 'tr' ? 'Kredi Kartı' : 'Credit Card');
  const [success, setSuccess] = useState(false);

  const PAYMENT_METHODS = language === 'tr'
    ? ['Nakit','Kredi Kartı','EFT/Havale','Acente Faturası','Açık Hesap']
    : ['Cash','Credit Card','Bank Transfer','Agency Invoice','Open Account'];
      ...
      <motion.div className="pay-modal" onClick={e => e.stopPropagation()} initial={{scale:.9,y:20}} animate={{scale:1,y:0}}>
        {success ? (
          <div className="pay-success">
            ...
            <h3>{language === 'tr' ? 'Ödeme Alındı!' : 'Payment Received!'}</h3>
            <p>₺{Number(amount).toLocaleString()} — {method}</p>
          </div>
        ) : (
          <>
            <div className="modal-head">
              <h3>{language === 'tr' ? 'Tahsilat Al' : 'Collect Payment'} — {res.guest}</h3>
              <button onClick={onClose}><X size={20}/></button>
            </div>
            <div className="pay-body">
              <div className="balance-badge">
                <AlertCircle size={20}/> {language === 'tr' ? 'Kalan Bakiye' : 'Remaining Balance'}: <strong>₺{res.balance.toLocaleString()}</strong>
              </div>
            ...
            <div className="modal-foot">
              <button className="btn-cancel" onClick={onClose}>{language === 'tr' ? 'İptal' : 'Cancel'}</button>
              <button className="btn-pay-confirm" onClick={handlePay} disabled={!amount || amount <= 0}>
                <CreditCard size={16}/> ₺{Number(amount||0).toLocaleString()} {language === 'tr' ? 'Tahsil Et' : 'Collect'}
              </button>
            </div>
          </>
        )}
...
const AddExtraModal = ({ resId, onClose, onAdd, language }) => {
  const EXTRA_ITEMS = language === 'tr' ? [
    { desc:'Minibar', amount:0, type:'extra' },
    { desc:'Restoran — Öğle', amount:450, type:'extra' },
    { desc:'Restoran — Akşam', amount:850, type:'extra' },
    { desc:'SPA — Masaj', amount:600, type:'extra' },
    { desc:'Oda Servisi', amount:0, type:'extra' },
    { desc:'Çamaşır Hizmeti', amount:150, type:'extra' },
    { desc:'Transfer', amount:500, type:'extra' },
    { desc:'Diğer', amount:0, type:'extra' },
  ] : [
    { desc:'Minibar', amount:0, type:'extra' },
    { desc:'Restaurant — Lunch', amount:450, type:'extra' },
    { desc:'Restaurant — Dinner', amount:850, type:'extra' },
    { desc:'SPA — Massage', amount:600, type:'extra' },
    { desc:'Room Service', amount:0, type:'extra' },
    { desc:'Laundry', amount:150, type:'extra' },
    { desc:'Transfer', amount:500, type:'extra' },
    { desc:'Other', amount:0, type:'extra' },
  ];
  ...
  const handleSelect = (item) => { setSelected(item); setDesc(item.desc); setAmount(item.amount); };
  const handleAdd = () => { if (!desc || !amount) return; onAdd(resId, { desc, amount: Number(amount), type: selected.type }); onClose(); };

  return (
    <motion.div className="modal-overlay" onClick={onClose} initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}}>
      <motion.div className="add-extra-modal" onClick={e => e.stopPropagation()} initial={{scale:.9,y:20}} animate={{scale:1,y:0}}>
        <div className="modal-head">
          <h3>{language === 'tr' ? 'Ekstra Tahakkuk Ekle' : 'Add Extra Charge'}</h3>
          <button onClick={onClose}><X size={20}/></button>
        </div>
        <div className="extra-body">
          ...
          <div className="extra-form">
            ...
            <button className="btn-add-confirm" onClick={handleAdd} disabled={!desc || !amount}>
              {language === 'tr' ? 'Tahakkuk Ekle' : 'Add Charge'}
            </button>
          </div>
        </div>
      </motion.div>
    ...
    <div className="folio-layout">
      ...
      <aside className="folio-sidebar">
        <div className="fs-head">
          <h3>{language === 'tr' ? 'İç Misafirler' : 'In-House Guests'}</h3>
          <div className="search-mini"><Search size={14}/><input placeholder={_c('search')} value={search} onChange={e => setSearch(e.target.value)}/></div>
        </div>
        ...
        {selectedRes ? (
          <>
            <div className="folio-header">
              <div>
                <h2>{_t('title')} — {selectedRes.guest}</h2>
                <span>{_c('room')} {selectedRes.room} · {selectedRes.checkIn} / {selectedRes.checkOut} · {selectedRes.board}</span>
              </div>
              <div className="fh-actions">
                <button className="btn-outline" onClick={() => window.print()}><Printer size={16}/> {language === 'tr' ? 'Yazdır' : 'Print'}</button>
                <button className="btn-extra" onClick={() => setShowExtraModal(true)}>
                  <Plus size={16}/> {language === 'tr' ? 'Ekstra Tahakkuk' : 'Add Charge'}
                </button>
                {selectedRes.balance > 0 && (
                  <button className="btn-pay" onClick={() => setShowPayModal(true)}>
                    <CreditCard size={16}/> {language === 'tr' ? 'Tahsilat Al' : 'Collect'} (₺{selectedRes.balance.toLocaleString()})
                  </button>
                )}
              </div>
            </div>
                ...
                <tbody>
                  {lines.map(line => (
                    <tr key={line.id}>
                      <td>{line.date}</td>
                      <td>{line.desc}</td>
                      <td><span className={`type-tag ${line.type}`}>{typeLabel(line.type)}</span></td>
            ...
            <div className="folio-summary">
              <div className="fs-row"><span>{_c('total')}</span><strong>₺{total.toLocaleString()}</strong></div>
              <div className="fs-row"><span>{language === 'tr' ? 'Ödenen' : 'Paid'}</span><strong className="green">₺{selectedRes.paid.toLocaleString()}</strong></div>
              <div className={`fs-row total ${selectedRes.balance > 0 ? 'due' : ''}`}>
                <span>{selectedRes.balance > 0 ? (language === 'tr' ? 'Kalan Borç' : 'Outstanding') : (language === 'tr' ? 'Hesap Durumu' : 'Account Status')}</span>
                <strong>{selectedRes.balance > 0 ? `₺${selectedRes.balance.toLocaleString()}` : `✓ ${_c('closed')}`}</strong>
              </div>
            </div>
          </>
        ) : (
          <div className="empty-state">
            ...
            <p>{language === 'tr' ? 'Soldaki listeden misafir seçin' : 'Select a guest from the list'}</p>
          </div>
        )}
      </main>
      ...
      <AnimatePresence>
        {showPayModal && selectedRes && (
          <PaymentModal res={selectedRes} onClose={() => setShowPayModal(false)} onPay={makePayment} language={language}/>
        )}
        {showExtraModal && selectedRes && (
          <AddExtraModal resId={selectedRes.id} onClose={() => setShowExtraModal(false)} onAdd={addFolioLine} language={language}/>
        )}
      </AnimatePresence>

      <style>{`
        .folio-layout { display:flex; height:calc(100vh - 70px); }
        .folio-sidebar { width:280px; background:white; border-right:1px solid #e2e8f0; display:flex; flex-direction:column; flex-shrink:0; }
        ...
        .btn-pay-confirm { padding:12px 24px; border-radius:12px; border:none; background:#3b82f6; color:white; font-weight:800; cursor:pointer; display:flex; align-items:center; gap:8px; }
