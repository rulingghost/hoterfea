...
import {
  ...
  ShoppingCart, TrendingDown, Archive, X
} from 'lucide-react';
...
const INITIAL_STOCK = [
  ...
  { id:'S-013', name:'Deterjan',         cat:'Temizlik', unit:'Lt',   stock:22,  minStock:10, cost:45 },
...
const Inventory = () => {
  ...
  const CAT_DISPLAY = { 'Banyo': isEn?'Bathroom':'Banyo', 'Çamaşır': isEn?'Laundry':'Çamaşır', 'Minibar':'Minibar', 'Oda': isEn?'Room':'Oda', 'Temizlik': isEn?'Cleaning':'Temizlik' };
  const catKeys = [...new Set(INITIAL_STOCK.map(i => i.cat))];
  const cats = ['all', ...catKeys];

  const filtered = items.filter(i => {
    const q = search.toLowerCase();
    return (catFilter === 'all' || i.cat === catFilter) && (!q || i.name.toLowerCase().includes(q));
  });
  ...
  const handleAdjust = () => {
    ...
    setItems(prev => prev.map(i => i.id === adjustModal.id
      ? {...i, stock: Math.max(0, adjustType === 'add' ? i.stock + qty : i.stock - qty)}
      : i));
    addNotification({ type:'info', msg: `${language === 'tr' ? 'Stok güncellendi' : 'Stock updated'}: ${adjustModal.name} (${adjustType === 'add' ? '+' : '-'}${qty})` });
    setAdjustModal(null);
  };

  const quickOrder = (item) => {
    setItems(prev => prev.map(i => i.id === item.id ? {...i, stock: i.stock + 50} : i));
    addNotification({ type:'success', msg: `${language === 'tr' ? 'Satın alma emri verildi' : 'Purchase order created'}: ${item.name} (+50)` });
  };
    ...
    <div className="inv-page">
      ...
      {lowStock.length > 0 && (
        <div className="low-alert">
          ...
          <span><strong>{lowStock.length} {language === 'tr' ? 'ürün' : 'items'}</strong> {language === 'tr' ? 'minimum stok seviyesinin altında' : 'below minimum stock level'}: {lowStock.map(i => i.name).join(', ')}</span>
        </div>
      )}

      <div className="inv-kpi">
        <div className="ik"><TrendingDown size={20} color="#ef4444"/><div><strong>{lowStock.length}</strong><span>{language === 'tr' ? 'Kritik Stok' : 'Critical Stock'}</span></div></div>
        <div className="ik"><Package size={20} color="#3b82f6"/><div><strong>{items.length}</strong><span>{language === 'tr' ? 'Ürün Çeşidi' : 'Product Types'}</span></div></div>
        <div className="ik"><Archive size={20} color="#10b981"/><div><strong>{items.reduce((s,i) => s + i.stock, 0)}</strong><span>{language === 'tr' ? 'Toplam Stok' : 'Total Stock'}</span></div></div>
        <div className="ik"><ShoppingCart size={20} color="#f59e0b"/><div><strong>₺{items.reduce((s,i) => s + i.stock * i.cost, 0).toLocaleString()}</strong><span>{language === 'tr' ? 'Stok Değeri' : 'Stock Value'}</span></div></div>
      </div>

      <div className="inv-filters">
        <div className="search-box"><Search size={14}/><input placeholder={_c('search')} value={search} onChange={e => setSearch(e.target.value)}/></div>
        <div className="cat-pills">
          {cats.map(c => (
            <button key={c} className={`cpill ${catFilter === c ? 'active' : ''}`} onClick={() => setCat(c)}>
              {c === 'all' ? _c('all') : (CAT_DISPLAY[c] || c)}
            </button>
          ))}
        </div>
      </div>
      ...
      <div className="inv-table-wrap">
        <table className="inv-table">
          <thead><tr>
            <th>{language === 'tr' ? 'Ürün Adı' : 'Product'}</th>
            <th>{_c('category')}</th>
            <th>{language === 'tr' ? 'Birim' : 'Unit'}</th>
            <th>{language === 'tr' ? 'Stok' : 'Stock'}</th>
            <th>{language === 'tr' ? 'Min. Stok' : 'Min. Stock'}</th>
            <th>{language === 'tr' ? 'Birim Maliyet' : 'Unit Cost'}</th>
            ...
            {filtered.map((item, i) => {
              ...
              return (
                <motion.tr key={item.id} initial={{opacity:0}} animate={{opacity:1}} transition={{delay:i*0.03}}>
                  ...
                  <td><span className="cat-badge">{CAT_DISPLAY[item.cat] || item.cat}</span></td>
                  <td>{item.unit}</td>
                  <td>
                  ...
                  <td>₺{item.cost}</td>
                  <td>
                    {critical ? <span className="tag red">{language === 'tr' ? 'Kritik' : 'Critical'}</span>
                     : low    ? <span className="tag orange">{language === 'tr' ? 'Düşük' : 'Low'}</span>
                              : <span className="tag green">{language === 'tr' ? 'Normal' : 'OK'}</span>}
                  </td>
                  <td>
                    <div className="act-btns">
                      <button className="act-btn blue" onClick={() => { setAdjustModal(item); setAdjustQty(10); setAdjustType('add'); }}>
                        {language === 'tr' ? 'Giriş/Çıkış' : 'Adjust'}
                      </button>
                      {low && <button className="act-btn orange" onClick={() => quickOrder(item)}>{language === 'tr' ? 'Sipariş Ver' : 'Order'}</button>}
                    </div>
                  </td>
                </motion.tr>
              );
          ...
          <motion.div className="modal-overlay" onClick={() => setAdjustModal(null)} initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}}>
            <motion.div className="adj-modal" onClick={e => e.stopPropagation()} initial={{scale:.9}} animate={{scale:1}}>
              <div className="modal-head">
                <h3>{language === 'tr' ? 'Stok Hareketi' : 'Stock Adjustment'} — {adjustModal.name}</h3>
                <button onClick={() => setAdjustModal(null)}><X size={18}/></button>
              </div>
              <div className="adj-body">
                <div className="cur-stock">{language === 'tr' ? 'Mevcut Stok' : 'Current Stock'}: <strong>{adjustModal.stock} {adjustModal.unit}</strong></div>
                <div className="type-toggle">
                  <button className={adjustType === 'add' ? 'active add' : ''} onClick={() => setAdjustType('add')}><Plus size={15}/> {language === 'tr' ? 'Giriş (Artır)' : 'Add Stock'}</button>
                  <button className={adjustType === 'sub' ? 'active sub' : ''} onClick={() => setAdjustType('sub')}><Minus size={15}/> {language === 'tr' ? 'Çıkış (Azalt)' : 'Remove Stock'}</button>
                </div>
                <label>{language === 'tr' ? 'Miktar' : 'Quantity'} ({adjustModal.unit})</label>
                <input type="number" value={adjustQty} onChange={e => setAdjustQty(e.target.value)} min={1}/>
                <div className="new-stock">
                  {language === 'tr' ? 'Yeni Stok' : 'New Stock'}: <strong style={{color: adjustType === 'add' ? '#10b981' : '#ef4444'}}>
                    {Math.max(0, adjustType === 'add' ? adjustModal.stock + Number(adjustQty) : adjustModal.stock - Number(adjustQty))} {adjustModal.unit}
                  </strong>
                </div>
              </div>
              <div className="modal-foot">
                <button className="btn-cancel" onClick={() => setAdjustModal(null)}>{_c('cancel')}</button>
                <button className="btn-save" onClick={handleAdjust}>{_c('save')}</button>
              </div>
            </motion.div>
          </motion.div>
        )}
