...
const CRS = () => {
  ...
  const defaultLogs = isEn ? [
    { id:'CRS-441', action:'Reservation Transfer',     from:'Antalya',  to:'Istanbul',  guest:'Ahmet Yılmaz',  status:'success' },
    { id:'CRS-440', action:'Group Block Sharing',       from:'HQ',       to:'All Hotels',guest:'Tech-A Group',  status:'success' },
    { id:'CRS-439', action:'Availability Sync',         from:'Channel',  to:'CRS Hub',   guest:'System',        status:'success' },
  ] : [
  ...
  const [logForm, setLogForm] = useState({ action: isEn ? 'Availability Sync' : 'Müsaitlik Senkronizasyonu', from:'Channel', to:'CRS Hub', guest: isEn ? 'System' : 'Sistem' });
  ...
  const addLog = (e) => {
    ...
    addNotification({ type:'success', msg: isEn ? 'New transaction logged' : 'Yeni işlem kaydı eklendi' });
    setLogForm({ action: isEn ? 'Availability Sync' : 'Müsaitlik Senkronizasyonu', from:'Channel', to:'CRS Hub', guest: isEn ? 'System' : 'Sistem' });
  ...
  const handleRefresh = () => {
    ...
    setTimeout(() => {
      ...
      setRefreshing(false);
      addNotification({ type:'info', msg: isEn ? 'All hotel availabilities synchronized' : 'Tüm otel müsaitlikleri senkronize edildi' });
      idCounter.current++;
      setLogs(p => [{
        id: `CRS-${idCounter.current}`,
        action: isEn ? 'Manual Sync' : 'Manuel Senkronizasyon',
        from: 'Admin', to: 'All Hotels',
        guest: isEn ? 'System' : 'Sistem',
        status: 'success'
      }, ...p]);
    }, 1500);
  };
  ...
  const makeReservation = (hotelName) => {
    addNotification({ type:'success', msg: `${hotelName} ${isEn ? 'cross-reservation screen opening...' : 'üzerinden çapraz rezervasyon ekranına yönlendiriliyorsunuz...'}` });
  };
  ...
  return (
    <div className="crs-page">
      <div className="crs-head">
        <div>
          <h2><Globe size={20}/> {isEn ? 'Central Reservation System (CRS)' : 'Merkezi Rezervasyon Sistemi (CRS)'}</h2>
          <span>{isEn ? 'Cross-hotel reservations and centralized availability tracking' : 'Zincir oteller arası çapraz rezervasyon ve merkezi müsaitlik takibi'}</span>
        </div>
        <div className="head-actions">
          <button className="btn-secondary" onClick={() => setShowLogForm(true)}><Plus size={14}/> {isEn ? 'Add Manual Transaction' : 'Manuel İşlem Ekle'}</button>
          <button className={`btn-primary ${refreshing ? 'spin' : ''}`} onClick={handleRefresh}>
            <RefreshCw size={14}/> {refreshing ? (isEn ? 'Updating...' : 'Güncelleniyor...') : (isEn ? 'Quick Availability Check' : 'Hızlı Müsaitlik Sorgula')}
          </button>
        </div>
      </div>
        ...
        {showLogForm && (
          <motion.div className="modal-overlay" initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}} onClick={() => setShowLogForm(false)}>
            <motion.form className="modal-box" initial={{scale:0.9}} animate={{scale:1}} onClick={e => e.stopPropagation()} onSubmit={addLog}>
              <div className="mb-head"><h3>{isEn ? 'Manual CRS Transaction' : 'Manuel CRS İşlemi'}</h3><button type="button" onClick={() => setShowLogForm(false)}><X size={18}/></button></div>
              <div className="mf-grid">
                <div className="mf"><label>{isEn ? 'Transaction Type' : 'İşlem Türü'}</label><input value={logForm.action} onChange={e => setLogForm(p => ({...p,action:e.target.value}))} required/></div>
                <div className="mf"><label>{isEn ? 'Guest / Source' : 'Misafir / Kaynak'}</label><input value={logForm.guest} onChange={e => setLogForm(p => ({...p,guest:e.target.value}))} required/></div>
                <div className="mf"><label>{isEn ? 'From' : 'Nereden'}</label><input value={logForm.from} onChange={e => setLogForm(p => ({...p,from:e.target.value}))} required/></div>
                <div className="mf"><label>{isEn ? 'To' : 'Nereye'}</label><input value={logForm.to} onChange={e => setLogForm(p => ({...p,to:e.target.value}))} required/></div>
              </div>
              <div className="mf-foot"><button type="submit" className="btn-primary">{isEn ? 'Save Transaction' : 'İşlemi Kaydet'}</button></div>
            </motion.form>
          </motion.div>
        )}
      </AnimatePresence>
      ...
      <div className="crs-search-bar">
        <div className="search-box">
          ...
          <input
            placeholder={isEn ? 'Search hotel name or city...' : 'Otel adı veya şehir ara...'}
            value={search}
        ...
        <div className="search-filters">
          ...
          <div className="sf-item"><Users size={14}/> 2 {isEn ? 'Adults' : 'Yetişkin'}</div>
          <button className="btn-go" onClick={handleRefresh}><Zap size={14}/> {isEn ? 'Search' : 'Sorgula'}</button>
        </div>
      </div>

      <div className="crs-grid">
        {filteredHotels.map((h, i) => (
          <motion.div key={h.id} className="hotel-crs-card" initial={{opacity:0,y:20}} animate={{opacity:1,y:0}} transition={{delay:i*0.1}}>
            ...
            <div className="hcc-head">
              ...
              <div className="hcc-info">
                ...
                <span><MapPin size={12}/> {h.city} · {isEn ? 'Cap' : 'Kp'}: {h.capacity}</span>
              </div>
            </div>
            <div className="hcc-stats">
              ...
              <div className="hccs">
                <span>{isEn ? 'Available Rooms' : 'Müsait Oda'}</span>
                <strong className={`hccs-big ${h.availability < 5 ? 'critical' : ''}`}>{h.availability}</strong>
              </div>
            </div>
            <div className="hcc-bottom">
              <div className="hcc-price">
                <span>{isEn ? 'Starting from' : 'Başlayan Fiyat'}</span>
                <strong>₺{h.rate.toLocaleString()}</strong>
              </div>
              <button className="btn-res" onClick={() => makeReservation(h.name)}>{isEn ? 'Reserve' : 'Rezervasyon Yap'} <ArrowRight size={14}/></button>
            </div>
          </motion.div>
        ))}
      </div>
      ...
      <div className="crs-footer">
        <h3>{isEn ? 'Recent Central Transactions' : 'Son Merkezi İşlemler'}</h3>
        <div className="crs-log">
          {logs.map(log => (
            <div key={log.id} className="log-row">
              ...
              <div className="log-status"><CheckCircle size={14} color="#10b981"/> {isEn ? 'Success' : 'Başarılı'}</div>
