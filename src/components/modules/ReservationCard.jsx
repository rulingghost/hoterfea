...
const ReservationCard = () => {
  ...
  const STATUS_MAP = {
    'check-in':  { label: isEn ? 'In House'    : 'İç Misafir',  color: '#10b981', bg: '#f0fdf4' },
    'check-out': { label: isEn ? 'Checked Out' : 'Çıkış Yaptı', color: '#64748b', bg: '#f8fafc' },
    'gelecek':   { label: isEn ? 'Upcoming'    : 'Gelecek',      color: '#3b82f6', bg: '#eff6ff' },
  };
    ...
    <div className="rc-page">
      ...
      <aside className="rc-sidebar">
        <div className="rcs-head">
          <h3>{isEn ? 'Reservation Cards' : 'Rezervasyon Kartları'}</h3>
          <span>{reservations.length} {isEn ? 'records' : 'kayıt'}</span>
        </div>
        <div className="rcs-search">
          <Search size={15} color="#94a3b8"/>
          <input placeholder={isEn ? 'Guest, room, no...' : 'Misafir, oda, no...'} value={search} onChange={e => setSearch(e.target.value)}/>
        </div>
            ...
            return (
              ...
              >
                ...
                <div className="rcs-info">
                  ...
                  <span>{r.id} · {isEn ? 'Room' : 'Oda'} {r.room || '—'}</span>
                </div>
                <span className="rcs-status" style={{ background: st.bg, color: st.color }}>{st.label}</span>
              </button>
            );
          })}
          {filtered.length === 0 && <p className="empty-msg">{isEn ? 'No results found.' : 'Sonuç bulunamadı.'}</p>}
        </div>
      </aside>
        ...
        {selected ? (
          <div className="rcd-inner">
            ...
            <div className="rc-strip">
              <div className="rcs-item-strip">
                ...
                <div className="rcsi-content">
                  ...
                  <span>{guest?.loyalty || (isEn ? 'Guest' : 'Misafir')} · {guest?.visits || 1} {isEn ? 'stays' : 'konaklama'}</span>
                </div>
              </div>
              <div className="rcs-item-strip">
                ...
                <div className="rcsi-content">
                  ...
                  <span>{selected.nights} {isEn ? 'Nights' : 'Gece'} · {selected.pax} {isEn ? 'Pax' : 'Kişi'}</span>
                </div>
              </div>
              <div className="rcs-item-strip">
                <div className="rcsi-icon"><Briefcase size={20}/></div>
                <div className="rcsi-content">
                  <strong>{isEn ? 'Room' : 'Oda'} {selected.room || (isEn ? 'Not Assigned' : 'Atanmadı')} — {selected.type}</strong>
            ...
            <div className="rc-tabs">
              {[
                { id:'genel',  label: isEn ? 'General Info'     : 'Genel Bilgiler',    icon: <Info size={15}/> },
                { id:'folio',  label: isEn ? 'Folio & Payments' : 'Folio & Ödemeler', icon: <CreditCard size={15}/> },
                { id:'notlar', label: isEn ? 'Notes'            : 'Notlar',            icon: <FileText size={15}/> },
              ].map(t => (
                <button key={t.id} className={`rc-tab ${activeTab === t.id ? 'active' : ''}`} onClick={() => setActiveTab(t.id)}>
                  {t.icon} {t.label}
                </button>
              ))}
            </div>
              ...
              {activeTab === 'genel' && (
                <div className="rc-grid">
                  <div className="rc-box">
                    <h4>{isEn ? 'Contact Info' : 'İletişim Bilgileri'}</h4>
                    <div className="rc-info-list">
                      <div className="ril-i"><Phone size={14}/> {guest?.phone || (isEn ? 'Not specified' : 'Belirtilmedi')}</div>
                      <div className="ril-i"><Mail size={14}/> {guest?.email || (isEn ? 'Not specified' : 'Belirtilmedi')}</div>
                      <div className="ril-i"><MapPin size={14}/> {guest?.nationality || 'TR'}</div>
                      {guest?.tcNo && <div className="ril-i"><ShieldCheck size={14}/> {isEn ? 'ID No' : 'TC'}: {guest.tcNo}</div>}
                      {guest?.passport && <div className="ril-i"><ShieldCheck size={14}/> {isEn ? 'Passport' : 'Pasaport'}: {guest.passport}</div>}
                    </div>
                  </div>
                  <div className="rc-box">
                    <h4>{isEn ? 'Pricing Summary' : 'Fiyatlandırma Özeti'}</h4>
                    <div className="rc-price-grid">
                      <div className="rpg-i"><span>{isEn ? 'Total' : 'Toplam Tutar'}</span><strong>₺{selected.total.toLocaleString()}</strong></div>
                      <div className="rpg-i"><span>{isEn ? 'Paid' : 'Ödenen'}</span><strong className="green">₺{selected.paid.toLocaleString()}</strong></div>
                      <div className="rpg-i divider">
                        <span>{isEn ? 'Outstanding' : 'Kalan Borç'}</span>
                        <strong className={selected.balance > 0 ? 'red' : 'green'}>
                          {selected.balance > 0 ? `₺${selected.balance.toLocaleString()}` : `✓ ${isEn ? 'Closed' : 'Kapandı'}`}
                        </strong>
                      </div>
                    </div>
                  </div>
                ...
                <div className="folio-tab">
                  <table className="mini-table">
                    <thead><tr>
                      ...
                      <th className="right">{isEn ? 'Amount' : 'Tutar'}</th>
                    </tr></thead>
                    <tbody>
                      {lines.map(l => (
                        <tr key={l.id}>
                          <td>{l.desc}</td>
                          <td><span className={`type-tag ${l.type}`}>{l.type === 'accommodation' ? (isEn ? 'Accommodation' : 'Konaklama') : (isEn ? 'Extra' : 'Ekstra')}</span></td>
                          <td>{l.date}</td>
                          <td className="right">₺{l.amount.toLocaleString()}</td>
                        </tr>
                      ))}
                      {lines.length === 0 && <tr><td colSpan={4} className="empty-msg">{isEn ? 'No folio entries yet.' : 'Henüz folio kaydı yok.'}</td></tr>}
                    </tbody>
                  </table>
                  <div className="folio-summary">
                    <div className="fs-row"><span>{isEn ? 'Folio Total' : 'Folio Toplam'}</span><strong>₺{folioTotal.toLocaleString()}</strong></div>
                    <div className="fs-row"><span>{isEn ? 'Paid' : 'Ödenen'}</span><strong className="green">₺{selected.paid.toLocaleString()}</strong></div>
                    <div className="fs-row big"><span>{isEn ? 'Balance' : 'Bakiye'}</span>
                      <strong className={selected.balance > 0 ? 'red' : 'green'}>
                        {selected.balance > 0 ? `₺${selected.balance.toLocaleString()}` : `✓ ${isEn ? 'Closed' : 'Kapandı'}`}
                      </strong>
                    </div>
              ...
              {activeTab === 'notlar' && (
                <div className="notes-tab">
                  <div className="note-box">
                    <h4>{isEn ? 'Reservation Notes' : 'Rezervasyon Notu'}</h4>
                    <p>{selected.notes || (isEn ? 'No notes for this reservation.' : 'Bu rezervasyona ait not bulunmuyor.')}</p>
                  </div>
                  {guest && (
                    <div className="note-box">
                      <h4>{isEn ? 'Guest History' : 'Misafir Geçmişi'}</h4>
                      <div className="guest-history">
                        <div><span>{isEn ? 'Total Stays' : 'Toplam Konaklama'}</span><strong>{guest.visits}</strong></div>
                        <div><span>{isEn ? 'Total Spent' : 'Toplam Harcama'}</span><strong>₺{guest.totalSpent.toLocaleString()}</strong></div>
                        <div><span>{isEn ? 'Last Visit' : 'Son Ziyaret'}</span><strong>{guest.lastVisit}</strong></div>
                        <div><span>{isEn ? 'Loyalty' : 'Sadakat'}</span><strong>{guest.loyalty}</strong></div>
                      </div>
                    </div>
                  )}
                </div>
            ...
            <div className="rc-footer">
              {selected.status === 'gelecek' && (
                <button className="btn-action green" onClick={() => { checkIn(selected.id); addNotification({ type:'success', msg:`Check-in: ${selected.guest}` }); }}>
                  <LogIn size={15}/> {isEn ? 'Check-In' : 'Check-In Yap'}
                </button>
              )}
              {selected.status === 'check-in' && (
                <button className="btn-action red" onClick={() => { checkOut(selected.id); addNotification({ type:'info', msg:`Check-out: ${selected.guest}` }); setSelectedResId(null); }}>
                  <LogOut size={15}/> {isEn ? 'Check-Out' : 'Check-Out Yap'}
                </button>
              )}
            </div>
          </div>
          ...
          <div className="rc-empty">
            ...
            <p>{isEn ? 'Select a reservation from the list to view details' : 'Detayları görmek için soldaki listeden bir rezervasyon seçin'}</p>
          </div>
        )}
      </main>
