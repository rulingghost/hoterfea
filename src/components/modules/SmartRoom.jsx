...
import { 
  ...
  Wind, Power, PowerOff, Battery, Search
} from 'lucide-react';
...
const SmartRoom = () => {
  ...
  const toggleControl = (roomId, controlType) => {
    ...
    addNotification({ type:'info', msg: isEn
      ? `Room ${roomId}: ${controlType} status changed`
      : `Oda ${roomId}: ${controlType} durumu değiştirildi`
    });
  };
  ...
  const FILTERS = [
    ...
    { val:'dolu',   labelTr:'Dolu',   labelEn:'Occupied'  },
    { val:'boş',    labelTr:'Boş',    labelEn:'Vacant'    },
    { val:'açık',   labelTr:'Açık',   labelEn:'Power On'  },
    { val:'kapalı', labelTr:'Kapalı', labelEn:'Power Off' },
  ];
  ...
  const energyData = [
    { name: isEn ? 'Occupied+On' : 'Dolu+Açık',  value: smartStatus.filter(r=>r.status==='dolu'&&r.controls.power).length },
    { name: isEn ? 'Vacant+On'   : 'Boş+Açık',   value: emptyPower },
    { name: isEn ? 'Off'         : 'Kapalı',       value: smartStatus.filter(r=>!r.controls.power).length },
  ];
  ...
  const shutdownEmpty = () => {
    ...
    if(Object.keys(updates).length > 0) {
      ...
      addNotification({ type:'success', msg: isEn
        ? `Energy shut down in ${Object.keys(updates).length} vacant rooms`
        : `${Object.keys(updates).length} boş odanın enerjisi kapatıldı`
      });
    }
  };

  const occLabel = (status) => {
    if (status === 'dolu')    return isEn ? '● Occupied' : '● İçeride';
    if (status === 'arızalı') return isEn ? '● OOO'      : '● Arızalı';
    return isEn ? '○ Vacant' : '○ Boş';
  };
  ...
  return (
    <div className="smart-page">
      <div className="smart-head">
        <div>
          <h2><Zap size={20}/> {isEn ? 'Smart Room & Energy Management' : 'Akıllı Oda & Enerji Yönetimi'}</h2>
          <span>{isEn ? 'IoT integration, energy saving and central control' : 'IoT entegrasyonu, enerji tasarrufu ve merkezi kontrol'}</span>
        </div>
        <div className="head-stats">
          <div className="hs-i"><Power size={14}/> <strong>{totalPower}</strong> {isEn ? 'Rooms On' : 'Oda Açık'}</div>
          <div className="hs-i"><Battery size={14}/> <strong>{avgTemp}°C</strong> {isEn ? 'Avg. Temp' : 'Ort. Sıcaklık'}</div>
          <button className="hs-btn" onClick={shutdownEmpty}>
            <PowerOff size={14}/> {isEn ? `Shut Off Vacant (${emptyPower})` : `Boş Odaları Kapat (${emptyPower})`}
          </button>
        </div>
      </div>
      ...
      <div className="energy-overview">
        <div className="eo-chart">
          <h4>{isEn ? 'Energy Distribution' : 'Enerji Dağılımı'}</h4>
        ...
        {savings > 0 && (
          <div className="eo-warn">
            ...
            <span>
              {isEn
                ? <><strong>{emptyPower} vacant rooms</strong> are consuming energy. Use "Shut Off Vacant" to save ~{savings}%.</>
                : <><strong>{emptyPower} boş oda</strong>da enerji harcıyorsunuz. "Boş Odaları Kapat" ile %{savings} tasarruf sağlayabilirsiniz.</>
              }
            </span>
          </div>
        )}
      ...
      <div className="smart-controls">
        <div className="search-bar">
          ...
          <input placeholder={isEn ? 'Search room no...' : 'Oda no ara...'} value={search} onChange={e=>setSearch(e.target.value)}/>
        </div>
        <div className="filters">
          {FILTERS.map(f => (
            <button key={f.val} className={`f-pill ${filter===f.val?'active':''}`} onClick={()=>setFilter(f.val)}>
              {isEn ? f.labelEn : f.labelTr} ({f.val==='all'?smartStatus.length:smartStatus.filter(r=>{
                if(f.val==='dolu')   return r.status==='dolu';
                if(f.val==='boş')    return r.status==='boş';
                if(f.val==='açık')   return r.controls.power;
                if(f.val==='kapalı') return !r.controls.power;
                return true;
              }).length})
            </button>
          ))}
        ...
        {filtered.map((r, i) => (
          ...
          >
            <div className="src-top">
              <div className="src-id">{isEn ? 'Room' : 'Oda'} {r.id}</div>
            ...
            <div className="src-main">
              ...
              <div className="src-actions">
                <button className={`src-a-btn ${r.controls.light?'active':''}`} title={isEn?'Lighting':'Aydınlatma'} onClick={()=>toggleControl(r.id,'light')}>
                  <Lightbulb size={16}/>
                </button>
                <button className={`src-a-btn ${r.controls.lock?'active':''}`} title={isEn?'Lock':'Kilit'} onClick={()=>toggleControl(r.id,'lock')}>
                  {r.controls.lock ? <Lock size={16}/> : <Unlock size={16}/>}
                </button>
                <button className={`src-a-btn ${r.controls.ac?'active':''}`} title={isEn?'AC':'Klima'} onClick={()=>toggleControl(r.id,'ac')}>
                  <Wind size={16}/>
                </button>
              </div>
            </div>
            ...
            <div className="src-foot">
              <button className={`power-toggle ${r.controls.power?'on':''}`} onClick={()=>toggleControl(r.id,'power')}>
                ...
                <span>{r.controls.power ? (isEn?'Active':'Aktif') : (isEn?'Off':'Kapalı')}</span>
              </button>
