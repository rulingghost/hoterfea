...
const ITInfrastructure = () => {
  ...
  const INITIAL_SYSTEMS = isEn ? [
    { id:'SYS-001', name:'Main Server (ERP)',        status:'online',  load:12, uptime:'142 days', icon:'Server' },
    { id:'SYS-002', name:'Bank Integration Gateway', status:'online',  load:5,  uptime:'15 days',  icon:'Shield' },
    { id:'SYS-003', name:'Wi-Fi Network (Guest)',    status:'online',  load:68, uptime:'4 days',   icon:'Wifi' },
    { id:'SYS-004', name:'Wi-Fi Network (Staff)',    status:'online',  load:42, uptime:'4 days',   icon:'Wifi' },
    { id:'SYS-005', name:'Database (Azure)',          status:'warning', load:85, uptime:'240 days', icon:'Database' },
    { id:'SYS-006', name:'Backup Unit (NAS)',         status:'online',  load:2,  uptime:'12 days',  icon:'HardDrive' },
  ] : [
  ...
  const handleRefresh = () => {
    ...
    setTimeout(() => {
      ...
      setRefreshing(false);
      addNotification({ type:'info', msg: isEn ? 'System statuses refreshed' : 'Sistem durumları güncellendi' });
    }, 1500);
  };
  ...
  const toggleSystem = (id) => {
    ...
    addNotification({ type: sys?.status === 'online' ? 'warn' : 'success', msg: `${sys?.name} ${sys?.status === 'online' ? (isEn ? 'stopped' : 'kapatıldı') : (isEn ? 'started' : 'başlatıldı')}` });
  };

  const restartSystem = (id) => {
    const sys = systems.find(s => s.id === id);
    setSystems(p => p.map(s => s.id === id ? { ...s, status: 'restarting', load: 0 } : s));
    addNotification({ type:'info', msg: `${sys?.name} ${isEn ? 'restarting...' : 'yeniden başlatılıyor...'}` });
    setTimeout(() => {
      setSystems(p => p.map(s => s.id === id ? { ...s, status: 'online', load: Math.floor(Math.random() * 30), uptime: isEn ? '0 days' : '0 gün' } : s));
      addNotification({ type:'success', msg: `${sys?.name} ${isEn ? 'restarted' : 'yeniden başlatıldı'}` });
    }, 2000);
  };
  ...
  const addSystem = (e) => {
    ...
    setSystems(p => [...p, { ...form, id, status:'online', load:0, uptime: isEn ? '0 days' : '0 gün' }]);
    addNotification({ type:'success', msg: `${isEn ? 'New system added' : 'Yeni sistem eklendi'}: ${form.name}` });
    setForm({ name:'', icon:'Server' });
    setShowForm(false);
  };

  const deleteSystem = (id) => setSystems(p => p.filter(s => s.id !== id));

  const statusText = (status) => {
    if (status === 'online')     return isEn ? '● Running' : '● Çalışıyor';
    if (status === 'warning')    return isEn ? '⚠ High Load' : '⚠ Yüksek Yük';
    if (status === 'restarting') return isEn ? '↻ Restarting' : '↻ Yeniden Başlatılıyor';
    return isEn ? '● Offline' : '● Kapalı';
  };
  ...
  return (
    <div className="it-page">
      <div className="it-head">
        <div>
          <h2><Activity size={20}/> {isEn ? 'IT Infrastructure & System Monitor' : 'IT Altyapı & Sistem İzleme'}</h2>
          <span>{isEn ? 'Real-time status of hardware, network and software services' : 'Donanım, ağ ve yazılım servislerinin gerçek zamanlı durumu'}</span>
        </div>
        <div style={{display:'flex',gap:10}}>
          <button className="btn-add" onClick={() => setShowForm(true)}><Plus size={14}/> {isEn ? 'Add System' : 'Sistem Ekle'}</button>
          <button className={`btn-refresh ${refreshing ? 'spin' : ''}`} onClick={handleRefresh}><RefreshCw size={15}/></button>
        </div>
      </div>

      <div className="it-kpi">
        <div className="kpi-item"><span>{isEn ? 'Running' : 'Çalışan'}</span><strong style={{color:'#10b981'}}>{onlineCount}</strong></div>
        <div className="kpi-item"><span>{isEn ? 'Warning' : 'Uyarı'}</span><strong style={{color:'#ef4444'}}>{warningCount}</strong></div>
        <div className="kpi-item"><span>{isEn ? 'Total' : 'Toplam'}</span><strong>{systems.length}</strong></div>
      </div>
          ...
          <motion.div className="modal-overlay" initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}} onClick={() => setShowForm(false)}>
            <motion.form className="modal-box" initial={{scale:0.9}} animate={{scale:1}} onClick={e => e.stopPropagation()} onSubmit={addSystem}>
              <div className="mb-head">
                <h3>{isEn ? 'Add New System' : 'Yeni Sistem Ekle'}</h3>
                <button type="button" onClick={() => setShowForm(false)}><X size={18}/></button>
              </div>
              <div className="mf-grid">
                <div className="mf"><label>{isEn ? 'System Name *' : 'Sistem Adı *'}</label><input value={form.name} onChange={e => setForm(p => ({...p,name:e.target.value}))} required placeholder="e.g. Redis Cache"/></div>
                <div className="mf"><label>{isEn ? 'Icon' : 'İkon'}</label><select value={form.icon} onChange={e => setForm(p => ({...p,icon:e.target.value}))}>{Object.keys(ICONS).map(k => <option key={k}>{k}</option>)}</select></div>
              </div>
              <div className="mf-foot"><button type="submit" className="btn-add">{_c('add')}</button></div>
            </motion.form>
          </motion.div>
        )}
      </AnimatePresence>
      ...
      <div className="it-grid">
        <div className="systems-list">
          {systems.map((s, i) => (
            <motion.div key={s.id} className={`system-card ${s.status}`} initial={{opacity:0,y:10}} animate={{opacity:1,y:0}} transition={{delay:i*0.05}}>
              <div className="sc-top">
                <div className="sc-icon">{ICONS[s.icon] || <Server size={20}/>}</div>
                <div className="sc-btns">
                  <button className="sc-btn" onClick={() => restartSystem(s.id)} title={isEn ? 'Restart' : 'Yeniden Başlat'}><RefreshCw size={12}/></button>
                  <button className="sc-btn" onClick={() => toggleSystem(s.id)} title={s.status === 'online' ? (isEn ? 'Stop' : 'Kapat') : (isEn ? 'Start' : 'Başlat')}><Power size={12}/></button>
        ...
        <div className="it-sidebar">
          <div className="traffic-card">
            <h3>{isEn ? 'Internet Traffic' : 'İnternet Trafiği'}</h3>
            <div className="t-row">
          ...
          {warningCount > 0 && (
            <div className="alert-box">
              <div className="ab-head"><AlertTriangle size={18} color="#ef4444"/><strong>{isEn ? 'Critical Alerts' : 'Kritik Uyarılar'}</strong></div>
              {systems.filter(s => s.load > 80).map(s => (
                <p key={s.id}><strong>{s.name}</strong> — %{s.load} {isEn ? 'load detected.' : 'yük tespit edildi.'}</p>
              ))}
            </div>
          )}
        </div>
