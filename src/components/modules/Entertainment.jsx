...
const Entertainment = () => {
  ...
  const _t = (k) => mt('entertainment.' + k);
  const _c = (k) => mt('common.' + k);
  const { addNotification } = useHotel();
  const isEn = language === 'en';

  const CATEGORIES = isEn
    ? ['Sport', 'Fun', 'Kids', 'Show', 'Music', 'Culture', 'Spa']
    : ['Spor', 'Eğlence', 'Çocuk', 'Show', 'Müzik', 'Kültür', 'Spa'];

  const LOCATIONS = isEn
    ? ['Beach Garden', 'Kids Club', 'Main Pool', 'Beach Area', 'Amphitheater', 'Lobby Stage', 'Fitness Hall', 'Terrace', 'Restaurant']
    : ['Sahil Bahçesi', 'Kids Club', 'Ana Havuz', 'Plaj Sahası', 'Amfi Tiyatro', 'Lobi Sahne', 'Fitness Salonu', 'Teras', 'Restoran'];

  const DAYS = isEn ? ['Today', 'Tomorrow'] : ['Bugün', 'Yarın'];
  const DAY_FILTERS = isEn ? ['Today', 'Tomorrow', 'All'] : ['Bugün', 'Yarın', 'Tümü'];
  ...
  const [activities, setActivities] = useState([
    { id:'ACT-001', nameTr:'Sabah Yogası',          nameEn:'Morning Yoga',          time:'09:00', endTime:'10:00', locTr:'Sahil Bahçesi', locEn:'Beach Garden', catTr:'Spor',     catEn:'Sport', status:'aktif',      participants:12, day:'Bugün' },
    { id:'ACT-002', nameTr:'Çocuk Kulübü: Boyama', nameEn:'Kids Club: Painting',   time:'10:30', endTime:'12:00', locTr:'Kids Club',     locEn:'Kids Club',    catTr:'Çocuk',    catEn:'Kids',  status:'aktif',      participants:8,  day:'Bugün' },
    { id:'ACT-003', nameTr:'Su Topu Turnuvası',     nameEn:'Water Polo Tournament', time:'14:30', endTime:'15:30', locTr:'Ana Havuz',     locEn:'Main Pool',    catTr:'Eğlence',  catEn:'Fun',   status:'aktif',      participants:24, day:'Bugün' },
    { id:'ACT-004', nameTr:'Voleybol Maçı',         nameEn:'Volleyball Match',      time:'16:00', endTime:'17:00', locTr:'Plaj Sahası',   locEn:'Beach Area',   catTr:'Spor',     catEn:'Sport', status:'bekliyor',   participants:0,  day:'Bugün' },
    { id:'ACT-005', nameTr:'Sihirbazlık Gösterisi', nameEn:'Magic Show',            time:'21:00', endTime:'22:00', locTr:'Amfi Tiyatro',  locEn:'Amphitheater', catTr:'Show',     catEn:'Show',  status:'bekliyor',   participants:0,  day:'Bugün' },
    { id:'ACT-006', nameTr:'Aqua Aerobik',           nameEn:'Aqua Aerobics',         time:'10:00', endTime:'10:45', locTr:'Ana Havuz',     locEn:'Main Pool',    catTr:'Spor',     catEn:'Sport', status:'bekliyor',   participants:0,  day:'Yarın' },
    { id:'ACT-007', nameTr:'Mini Disco',             nameEn:'Mini Disco',            time:'20:00', endTime:'21:00', locTr:'Kids Club',     locEn:'Kids Club',    catTr:'Çocuk',    catEn:'Kids',  status:'bekliyor',   participants:0,  day:'Yarın' },
  ]);

  const [shows, setShows] = useState([
    { id:'SH-001', title: isEn ? 'Turkish Night' : 'Türk Gecesi',       date: isEn ? 'Tomorrow' : 'Yarın',    icon:'🎭', time:'21:00' },
    { id:'SH-002', title:'Live Music (Jazz)',                              date: isEn ? 'Friday'   : 'Cuma',     icon:'🎵', time:'21:00' },
    { id:'SH-003', title: isEn ? 'Open Air Cinema' : 'Açık Hava Sineması', date: isEn ? 'Saturday' : 'Cumartesi',icon:'🎬', time:'21:30' },
  ]);

  const [form, setForm] = useState({ nameTr:'', nameEn:'', time:'', endTime:'', locIdx:0, catIdx:0, dayIdx:0 });
  ...
  const submitActivity = (e) => {
    ...
    const newAct = {
      id, nameTr:form.nameTr, nameEn:form.nameEn,
      time:form.time, endTime:form.endTime,
      locTr:LOCATIONS[form.locIdx] || LOCATIONS[0], locEn:LOCATIONS[form.locIdx] || LOCATIONS[0],
      catTr:CATEGORIES[form.catIdx] || CATEGORIES[0], catEn:CATEGORIES[form.catIdx] || CATEGORIES[0],
      status:'bekliyor', participants:0, day:DAYS[form.dayIdx] || DAYS[0]
    };
    setActivities(p => [...p, newAct]);
    addNotification({ type:'info', msg:`${isEn ? 'New activity added' : 'Yeni aktivite eklendi'}: ${isEn ? form.nameEn : form.nameTr}` });
    setForm({ nameTr:'', nameEn:'', time:'', endTime:'', locIdx:0, catIdx:0, dayIdx:0 });
    setShowForm(false);
  };
  ...
  const submitShow = (e) => {
    ...
    addNotification({ type:'success', msg:`${isEn ? 'Show added' : 'Şov eklendi'}: ${showFormData.title}` });
    setShowFormData({ title:'', date:'', icon:'🎭', time:'21:00' });
    setShowShowForm(false);
  };

  const startActivity    = (id) => { setActivities(p => p.map(a => a.id === id ? {...a, status:'aktif'} : a)); addNotification({ type:'success', msg: isEn ? 'Activity started' : 'Aktivite başlatıldı' }); };
  const completeActivity = (id) => { setActivities(p => p.map(a => a.id === id ? {...a, status:'tamamlandı'} : a)); addNotification({ type:'info', msg: isEn ? 'Activity completed' : 'Aktivite tamamlandı' }); };
  const addParticipant   = (id) => setActivities(p => p.map(a => a.id === id ? {...a, participants:a.participants+1} : a));
  const deleteActivity   = (id) => setActivities(p => p.filter(a => a.id !== id));
  const deleteShow       = (id) => setShows(p => p.filter(s => s.id !== id));

  // Map activities to active day (comparing with TR values since data uses TR)
  const todayStr = DAYS[0];
  const filteredActivities = activities.filter(a => {
    if (activeDay === (isEn ? 'All' : 'Tümü')) return true;
    const dayTr = isEn ? (activeDay === 'Today' ? 'Bugün' : 'Yarın') : activeDay;
    return a.day === dayTr;
  });
  ...
  return (
    <div className="ent-page">
      <div className="ent-head">
        <div>
          <h2><PartyPopper size={20}/> {isEn ? 'Entertainment & Activities' : 'Eğlence & Aktivite Yönetimi'}</h2>
          <span>{isEn ? 'Daily animation program, special shows and guest participation tracking' : 'Günlük animasyon programı, özel şovlar ve misafir katılım takibi'}</span>
        </div>
        <button className="btn-primary" onClick={() => setShowForm(true)}><Plus size={14}/> {isEn ? 'Add Activity' : 'Yeni Etkinlik Ekle'}</button>
      </div>

      <AnimatePresence>
        {showForm && (
          <motion.form className="act-form" onSubmit={submitActivity} initial={{opacity:0,y:-10}} animate={{opacity:1,y:0}} exit={{opacity:0}}>
            <div className="af-head"><h3>{isEn ? 'New Activity' : 'Yeni Aktivite'}</h3><button type="button" onClick={() => setShowForm(false)}><X size={18}/></button></div>
            <div className="af-grid">
              <input value={form.nameTr} onChange={e => setForm(p => ({...p,nameTr:e.target.value}))} placeholder="Aktivite adı (TR) *" required/>
              <input value={form.nameEn} onChange={e => setForm(p => ({...p,nameEn:e.target.value}))} placeholder="Activity name (EN) *" required/>
              ...
              <select value={form.locIdx} onChange={e => setForm(p => ({...p,locIdx:Number(e.target.value)}))}>
                {LOCATIONS.map((l,i) => <option key={i} value={i}>{l}</option>)}
              </select>
              <select value={form.catIdx} onChange={e => setForm(p => ({...p,catIdx:Number(e.target.value)}))}>
                {CATEGORIES.map((c,i) => <option key={i} value={i}>{c}</option>)}
              </select>
              <select value={form.dayIdx} onChange={e => setForm(p => ({...p,dayIdx:Number(e.target.value)}))}>
                {DAYS.map((d,i) => <option key={i} value={i}>{d}</option>)}
              </select>
            </div>
            <div className="af-foot"><button type="submit" className="btn-primary">{_c('add')}</button></div>
          </motion.form>
        )}
      </AnimatePresence>
        ...
        <div className="program-section">
          <div className="ps-head">
            ...
            <div className="ps-stats">
              <strong>{activeCount}</strong> {isEn ? 'Active' : 'Aktif'} · <strong>{totalParticipants}</strong> {isEn ? 'Participants' : 'Katılımcı'}
            </div>
          </div>

          <div className="activity-list">
            {filteredActivities.length === 0 && <div style={{textAlign:'center',padding:30,color:'#94a3b8',fontSize:13}}>{isEn ? 'No activities for this day' : 'Bu gün için aktivite yok'}</div>}
            {filteredActivities.map((a, i) => (
              <motion.div key={a.id} className={`act-card ${a.status}`} initial={{opacity:0,x:-20}} animate={{opacity:1,x:0}} transition={{delay:i*0.05}}>
                <div className="act-time"><Clock size={14}/> {a.time} - {a.endTime}</div>
                <div className="act-info">
                  <strong>{isEn ? a.nameEn : a.nameTr}</strong>
                  <span><MapPin size={11}/> {isEn ? a.locEn : a.locTr} · <Tag size={11}/> {isEn ? a.catEn : a.catTr}</span>
                </div>
                <div className="act-right">
                  <div className="act-users" onClick={() => addParticipant(a.id)} style={{cursor:'pointer'}} title={isEn ? 'Add participant' : 'Katılımcı ekle'}>
                    <Users size={12}/> {a.participants} {isEn ? 'Participants' : 'Katılımcı'}
                  </div>
                  <div className="act-btns">
                    {a.status === 'bekliyor'    && <button className="ab green"  onClick={() => startActivity(a.id)}>{isEn ? 'Start' : 'Başlat'}</button>}
                    {a.status === 'aktif'       && <button className="ab purple" onClick={() => completeActivity(a.id)}>{isEn ? 'End' : 'Bitir'}</button>}
        ...
        <div className="ent-sidebar">
          <div className="featured-shows">
            <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:16}}>
              <h3 style={{marginBottom:0}}>{isEn ? 'Weekly Special Shows' : 'Haftalık Özel Şovlar'}</h3>
            ...
            <AnimatePresence>
              {showShowForm && (
                <motion.form className="show-form" onSubmit={submitShow} initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}}>
                  <input value={showFormData.title} onChange={e => setShowFormData(p => ({...p,title:e.target.value}))} placeholder={isEn ? 'Show name *' : 'Şov adı *'} required/>
                  <input value={showFormData.date} onChange={e => setShowFormData(p => ({...p,date:e.target.value}))} placeholder={isEn ? 'Day (Friday...)' : 'Gün (Cuma, Pazar)'} required/>
                  <select value={showFormData.icon} onChange={e => setShowFormData(p => ({...p,icon:e.target.value}))}>
                    {SHOW_ICONS.map(i => <option key={i}>{i}</option>)}
                  </select>
                  <button type="submit" className="btn-primary sm">{_c('add')}</button>
                </motion.form>
              )}
            </AnimatePresence>
            ...
            <div className="show-list">
              {shows.map(s => (
                <div key={s.id} className="show-card">
                  ...
                  <button className="sc-rsvp" onClick={() => addNotification({type:'success',msg:`${isEn ? 'Show announced' : 'Şov duyuruldu'}: ${s.title}`})}>{isEn ? 'Announce' : 'Duyur'}</button>
          ...
          <div className="kids-club-box">
            <div className="kc-head">
              ...
              <h3>{isEn ? 'Kids Club Status' : 'Kids Club Durumu'}</h3>
            </div>
            <div className="kc-stats">
              <div className="ks-i">
                <span>{isEn ? 'Active Kids' : 'Aktif Çocuk'}</span>
                <strong>{activities.filter(a => (a.catTr === 'Çocuk') && a.status === 'aktif').reduce((s,a) => s+a.participants, 0)}</strong>
              </div>
              <div className="ks-i">
                <span>{isEn ? 'Activities' : 'Aktivite'}</span>
                <strong>{activities.filter(a => a.catTr === 'Çocuk').length}</strong>
              </div>
            </div>
          </div>

          <div className="staff-box">
            <h4>{isEn ? 'Team Status (Animation)' : 'Ekip Durumu (Animasyon)'}</h4>
            <div className="st-list">
              {[
                { name:'Ricardo', roleTr:'DJ / Teknik',      roleEn:'DJ / Technical' },
                { name:'Elena',   roleTr:'Yoga Eğitmeni',    roleEn:'Yoga Instructor' },
                { name:'Mert',    roleTr:'Çocuk Şefi',       roleEn:'Kids Chief' },
              ].map(st => (
                <div key={st.name} className="st-row">
                  <div className="st-av">{st.name[0]}</div>
                  <div className="st-info"><strong>{st.name}</strong><span>{isEn ? st.roleEn : st.roleTr}</span></div>
      ...
      <style>{`
        ...
        .d-tab{padding:6px 14px;border-radius:8px;border:none;font-size:11px;font-weight:700;color:#64748b;cursor:pointer;background:transparent;}
