...
import {
  ...
  Edit, X, Calendar, TrendingUp, User, Heart
} from 'lucide-react';
...
const GuestFormModal = ({ guest, onClose, onSave, language }) => {
    ...
    <motion.div className="modal-overlay" onClick={onClose} initial={{opacity:0}} animate={{opacity:1}} exit={{opacity:0}}>
      <motion.div className="gf-modal" onClick={e => e.stopPropagation()} initial={{scale:.9,y:20}} animate={{scale:1,y:0}}>
        <div className="modal-head">
          <h3>{guest ? (language === 'tr' ? 'Misafir Düzenle' : 'Edit Guest') : (language === 'tr' ? 'Yeni Misafir Kaydı' : 'New Guest')}</h3>
          <button onClick={onClose}><X size={20}/></button>
        </div>
        <div className="gf-body">
          <div className="form-grid">
            <div className="fg-group full"><label>{language === 'tr' ? 'Ad Soyad *' : 'Full Name *'}</label><input value={form.name} onChange={e => set('name', e.target.value)} placeholder={language === 'tr' ? 'Ad Soyad' : 'Full Name'}/></div>
            <div className="fg-group"><label>{language === 'tr' ? 'Uyruk' : 'Nationality'}</label>
            ...
            <div className="fg-group"><label>{language === 'tr' ? 'Sadakat Seviyesi' : 'Loyalty Level'}</label>
              <select value={form.loyalty} onChange={e => set('loyalty', e.target.value)}>
                {['None','Silver','Gold','Platinum'].map(l => <option key={l}>{l}</option>)}
              </select>
            </div>
            <div className="fg-group"><label>{language === 'tr' ? 'Telefon' : 'Phone'}</label><input value={form.phone} onChange={e => set('phone', e.target.value)} placeholder="+90..."/></div>
            <div className="fg-group"><label>E-mail</label><input value={form.email} onChange={e => set('email', e.target.value)} placeholder="email@example.com"/></div>
            <div className="fg-group"><label>{language === 'tr' ? 'Doğum Tarihi' : 'Date of Birth'}</label><input type="date" value={form.dob} onChange={e => set('dob', e.target.value)}/></div>
            <div className="fg-group"><label>{language === 'tr' ? 'TC Kimlik No' : 'ID Number'}</label><input value={form.tcNo} onChange={e => set('tcNo', e.target.value)} placeholder="12345678900"/></div>
            <div className="fg-group"><label>{language === 'tr' ? 'Pasaport No' : 'Passport No'}</label><input value={form.passport} onChange={e => set('passport', e.target.value)} placeholder="A1234567"/></div>
          </div>
        </div>
        <div className="modal-foot">
          ...
          <button className="btn-save" onClick={() => { onSave(form); onClose(); }} disabled={!form.name}>
            {guest ? (language === 'tr' ? 'Güncelle' : 'Update') : (language === 'tr' ? 'Misafir Oluştur' : 'Create Guest')}
          </button>
        </div>
      </motion.div>
    </motion.div>
...
const GuestDetailPanel = ({ guest, reservations, onClose, onEdit, language }) => {
  const guestRes = reservations.filter(r => r.guestId === guest.id || r.guest === guest.name);
  const lc = LOYALTY_COLORS[guest.loyalty];

  const statusLabel = (s) =>
    s === 'check-in'  ? (language === 'tr' ? 'İçeride' : 'In-House') :
    s === 'check-out' ? (language === 'tr' ? 'Çıktı'   : 'Checked Out') :
                        (language === 'tr' ? 'Bekliyor' : 'Upcoming');
  ...
  return (
    <motion.div className="detail-panel" initial={{x:40,opacity:0}} animate={{x:0,opacity:1}} exit={{x:40,opacity:0}}>
      ...
      <div className="dp-stats">
        <div className="dps"><Calendar size={16}/><div><strong>{guest.visits}</strong><span>{language === 'tr' ? 'Konaklama' : 'Stays'}</span></div></div>
        <div className="dps"><TrendingUp size={16}/><div><strong>₺{guest.totalSpent.toLocaleString()}</strong><span>{language === 'tr' ? 'Toplam Harcama' : 'Total Spent'}</span></div></div>
        <div className="dps"><Star size={16}/><div><strong>{guest.lastVisit}</strong><span>{language === 'tr' ? 'Son Ziyaret' : 'Last Visit'}</span></div></div>
      </div>
      ...
      <div className="dp-contact">
        ...
        {guest.passport && <div className="dc-row"><User size={14}/><span>{language === 'tr' ? 'Pasaport' : 'Passport'}: {guest.passport}</span></div>}
      </div>

      <div className="dp-section">
        <h4>{language === 'tr' ? 'Rezervasyon Geçmişi' : 'Reservation History'}</h4>
        {guestRes.length === 0
          ? <p className="no-res">{language === 'tr' ? 'Rezervasyon bulunamadı.' : 'No reservations found.'}</p>
          : guestRes.map(r => (
            <div key={r.id} className="res-mini">
              <div className="rm-left">
                <strong>{r.id}</strong>
                <span>{language === 'tr' ? 'Oda' : 'Room'} {r.room || '—'} · {r.checkIn} / {r.checkOut}</span>
              </div>
              <span className={`rmtag ${r.status}`}>{statusLabel(r.status)}</span>
            </div>
          ))
        }
      </div>

      <button className="dp-edit-btn" onClick={() => onEdit(guest)}>
        <Edit size={14}/> {language === 'tr' ? 'Misafiri Düzenle' : 'Edit Guest'}
      </button>
    </motion.div>
  );
};
...
const GuestCRM = () => {
  ...
  const filtered = guests.filter(g => {
    ...
    const matchL = loyaltyFilter === 'all' || g.loyalty === loyaltyFilter;
  ...
  return (
    <div className="crm-layout">
      ...
      <div className="crm-list-panel">
        <div className="cls-head">
          ...
          <button className="btn-primary" onClick={() => { setEditingGuest(null); setShowForm(true); }}>
            <Plus size={16}/> {language === 'tr' ? 'Yeni Misafir' : 'New Guest'}
          </button>
        </div>
        ...
        <div className="crm-mini-stats">
          ...
          <div className="cms"><strong>{guests.length}</strong><span>{language === 'tr' ? 'Toplam' : 'Total'}</span></div>
        </div>

        <div className="crm-filters">
          <div className="search-box">
            <Search size={15} color="#94a3b8"/>
            <input placeholder={language === 'tr' ? 'İsim, e-posta, telefon...' : 'Name, email, phone...'} value={search} onChange={e => setSearch(e.target.value)}/>
          </div>
          <div className="loyalty-filter">
            {['all','Platinum','Gold','Silver','None'].map(l => (
              <button key={l} className={`lf-btn ${loyaltyFilter === l ? 'active' : ''}`} onClick={() => setLoyaltyFilter(l)}>
                {l === 'all' ? _c('all') : l}
              </button>
            ))}
          </div>
        ...
        <div className="guest-cards">
          ...
          {filtered.length === 0 && (
            <p className="no-res" style={{padding:'30px',textAlign:'center',color:'#94a3b8'}}>
              {language === 'tr' ? 'Misafir bulunamadı.' : 'No guests found.'}
            </p>
          )}
        </div>
      </div>
        ...
        <AnimatePresence mode="wait">
          {selectedGuest ? (
            ...
            <motion.div className="no-selection" initial={{opacity:0}} animate={{opacity:1}}>
              ...
              <p>{language === 'tr' ? 'Misafir detayları için listeden birini seçin' : 'Select a guest to view details'}</p>
            </motion.div>
          )}
        </AnimatePresence>
      </div>
      ...
      <style>{`
        ...
        .crm-list-panel { width:440px; border-right:1px solid #e2e8f0; background:white; display:flex; flex-direction:column; overflow:hidden; flex-shrink:0; }
