...
const AIPricing = () => {
  ...
  const chartData = typeStats.map((s) => ({
    name: s.type,
    [isEn ? 'Current' : 'Mevcut']: s.base,
    [isEn ? 'AI Suggested' : 'AI Önerisi']: Math.round(s.base * (1 + s.aiSuggestion / 100)),
    [isEn ? 'Adjusted' : 'Ayarlanmış']: s.newPrice
  }));
  ...
  const handleApplyAll = () => {
    ...
    setTimeout(() => {
      ...
      addNotification({ type: 'success', msg: isEn
        ? `New pricing strategy (${totalDiff > 0 ? '+' : ''}₺${totalDiff.toLocaleString()} potential diff) applied to all channels!`
        : `Yeni fiyatlandırma stratejisi (${totalDiff > 0 ? '+' : ''}${totalDiff.toLocaleString()}₺ potansiyel fark) tüm kanallara başarıyla uygulandı!`
      });
  ...
  return (
    <div className="ai-pricing">
      <div className="pricing-top-bar">
        <div className="pricing-summary flex-1">
          <div className="ps-card">
            ...
            <div>
              <span>{isEn ? 'Current Revenue (Daily)' : 'Mevcut Gelir (Günlük)'}</span>
              <strong>₺{totalBaseRev.toLocaleString()}</strong>
            </div>
          </div>
          <div className="ps-card highlight">
            <TrendingUp size={20} color="#10b981"/>
            <div>
              <span>{isEn ? 'Adjusted Revenue' : 'Ayarlanmış Gelir'}</span>
              <strong>₺{totalNewRev.toLocaleString()}</strong>
            </div>
          </div>
          <div className={`ps-card ${totalDiff >= 0 ? 'up' : 'down'}`}>
            {totalDiff >= 0 ? <ArrowUpRight size={20}/> : <ArrowDownRight size={20}/>}
            <div>
              <span>{isEn ? 'Difference' : 'Fark'}</span>
        ...
        >
          {isApplying
            ? <><Loader2 size={18} className="spin"/> {isEn ? 'Updating System...' : 'Sistem Güncelleniyor...'}</>
            : <><Save size={18}/> {isEn ? 'Publish Prices' : 'Fiyatları Yayına Al'}</>
          }
        </button>
      </div>
        ...
        {typeStats.map((s, i) => (
          <motion.div key={s.type} className={`price-card ${s.adj !== 0 ? 'modified' : ''}`} initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.1 }}>
            <div className="pc-head">
              ...
              <h4>{s.type}</h4>
              <button className="lock-btn" onClick={() => setLocked(p => ({ ...p, [s.type]: !p[s.type] }))}>
                {locked[s.type] ? <Lock size={14} color="#ef4444"/> : <Unlock size={14}/>}
              </button>
            </div>
            <div className="pc-stats">
              <div><span>{isEn ? 'Occupancy' : 'Doluluk'}</span><strong>%{s.occRate}</strong></div>
              <div><span>{isEn ? 'Rooms' : 'Oda'}</span><strong>{s.occ}/{s.total}</strong></div>
              <div><span>{isEn ? 'Base Rate' : 'Baz Fiyat'}</span><strong>₺{s.base.toLocaleString()}</strong></div>
            </div>
            <div className={`pc-slider ${locked[s.type] ? 'locked-state' : ''}`}>
              <label>
                {isEn ? 'Price Adjustment' : 'Fiyat Ayarı'}:{' '}
                <strong style={{ color: s.adj >= 0 ? (s.adj===0 ? '#1e293b' : '#10b981') : '#ef4444' }}>
                  {s.adj > 0 ? '+' : ''}{s.adj}%
                </strong>
              </label>
            ...
            <div className="pc-result">
              <div className="new-price">
                {isEn ? 'New Price' : 'Yeni Fiyat'}: <strong>₺{s.newPrice.toLocaleString()}</strong>
              </div>
              <div className={`rev-diff ${s.diff >= 0 ? (s.diff===0?'':'up') : 'down'}`}>
                {s.diff > 0 ? '+' : ''}₺{s.diff.toLocaleString()}/{isEn ? 'day' : 'gün'}
              </div>
            </div>
            {s.aiSuggestion !== 0 && (
              <button className="ai-suggest-btn" onClick={() => applyAISuggestion(s.type)} disabled={locked[s.type]}>
                🤖 {isEn ? `AI Suggestion: ${s.aiSuggestion > 0 ? '+' : ''}${s.aiSuggestion}% Apply` : `AI Önerisi: ${s.aiSuggestion > 0 ? '+' : ''}${s.aiSuggestion}% Uygula`}
              </button>
            )}
          </motion.div>
        ))}
      ...
      <div className="pricing-chart-card">
        <h3>{isEn ? 'Price Comparison' : 'Fiyat Karşılaştırması'}</h3>
        <ResponsiveContainer width="100%" height={250}>
          <BarChart data={chartData} barGap={8}>
            ...
            <Bar dataKey={isEn ? 'AI Suggested' : 'AI Önerisi'} fill="#8b5cf6" radius={[6, 6, 0, 0]}/>
            <Bar dataKey={isEn ? 'Adjusted' : 'Ayarlanmış'} fill="#3b82f6" radius={[6, 6, 0, 0]}/>
          </BarChart>
        </ResponsiveContainer>
      </div>
