...
import { Moon, CheckCircle, AlertCircle, TrendingUp, Bed, DollarSign, Loader } from 'lucide-react';

const NightAudit = () => {
  const { mt, language } = useModuleTranslation();
  const _t = (k) => mt('nightAudit.' + k);
  ...
  const AUDIT_STEPS = [
    { id:1, name: language === 'tr' ? 'Açık Folio Kontrolü'   : 'Open Folio Check',    desc: language === 'tr' ? 'Tüm açık hesaplar kontrol ediliyor...' : 'Checking all open folios...' },
    { id:2, name: language === 'tr' ? 'Gece Sayacı'           : 'Night Counter',       desc: language === 'tr' ? 'Geceler bir ileri alınıyor...'          : 'Advancing night counter...' },
    { id:3, name: language === 'tr' ? 'Oda Durumu Raporu'     : 'Room Status Report',  desc: language === 'tr' ? 'Tüm oda durumları raporlanıyor...'       : 'Reporting all room statuses...' },
    { id:4, name: language === 'tr' ? 'Gelir Kaydı'           : 'Revenue Entry',       desc: language === 'tr' ? 'Günlük gelir işleniyor...'               : 'Processing daily revenue...' },
    { id:5, name: language === 'tr' ? 'KBS Bildirim Kontrolü' : 'State Report Check',  desc: language === 'tr' ? 'Polis listesi kontrol ediliyor...'        : 'Verifying state report list...' },
    { id:6, name: language === 'tr' ? 'Yedekleme'             : 'Backup',              desc: language === 'tr' ? 'Veriler yedekleniyor...'                  : 'Backing up data...' },
  ];

  const today = TODAY;
  const todayTx = cashTransactions.filter(t => t.date === today);
  const totalRevenue = todayTx.filter(t => t.type === 'gelir').reduce((s, t) => s + t.amount, 0);
  ...
  const runAudit = async () => {
    ...
    setRunning(false);
    setDone(true);
    addNotification({ type: 'success', msg: language === 'tr' ? 'Gece Raporu başarıyla tamamlandı!' : 'Night Audit completed successfully!' });
  };
  ...
  return (
    <div className="na-container">
      <div className="na-header">
        <div>
          <h2><Moon size={22}/> {_t('title')}</h2>
          <span>{today} — {_t('subtitle')}</span>
        </div>
        {!running && !done && (
          <button className="btn-start" onClick={runAudit}>
            <Moon size={16}/> {_t('startAudit')}
          </button>
        )}
        {done && (
          <button className="btn-start green" onClick={() => { setDone(false); setCompletedSteps([]); }}>
            ✓ {_t('closed')} — {language === 'tr' ? 'Yeni Rapor' : 'New Report'}
          </button>
        )}
      </div>
      ...
      <div className="na-summary">
        <div className="ns-card">
          ...
          <div><strong>₺{totalRevenue.toLocaleString()}</strong><span>{_t('totalRevenue')}</span></div>
        </div>
        <div className="ns-card">
          <TrendingUp size={20} color="#3b82f6"/>
          <div><strong>%{stats.occupancyRate}</strong><span>{_t('occupancyRate')}</span></div>
        </div>
        <div className="ns-card">
          <Bed size={20} color="#8b5cf6"/>
          <div><strong>{stats.inHouse}</strong><span>{_t('inHouse')}</span></div>
        </div>
        <div className="ns-card">
          ...
          <div>
            ...
            <span>{language === 'tr' ? 'Açık Folio' : 'Open Folios'}</span>
          </div>
        </div>
      </div>
      ...
      {openFolios.length > 0 && (
        <div className="open-folio-warn">
          ...
          <span>
            {language === 'tr'
              ? `Gece raporunu başlatmadan önce ${openFolios.length} açık folio kapatılmalıdır.`
              : `${openFolios.length} open folio(s) must be settled before starting the night audit.`}
          </span>
        </div>
      )}
      ...
      <div className="na-steps">
        <h3>{language === 'tr' ? 'İşlem Adımları' : 'Audit Steps'}</h3>
            ...
            return (
              <motion.div key={s.id} className={`audit-step ${isComplete ? 'done' : ''} ${isCurrent ? 'running' : ''}`}>
                ...
                <div className="as-info">
                  ...
                  {isComplete && <span className="done-label">{_c('done')} ✓</span>}
                </div>
              </motion.div>
            );
          })}
      ...
      {done && (
        <motion.div className="na-done" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
          ...
          <div>
            <h3>{language === 'tr' ? 'Gece Raporu Başarıyla Tamamlandı!' : 'Night Audit Completed Successfully!'}</h3>
            <p>{today} — {language === 'tr' ? 'Kapanış kaydedildi. Sistem yeni güne hazır.' : 'Closing recorded. System is ready for the new day.'}</p>
          </div>
        </motion.div>
      )}
